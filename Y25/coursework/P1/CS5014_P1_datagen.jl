...
# ╔═╡ 337fa83f-4910-4f74-9524-ada9a97b8662
begin
	using Random
	...
	nobs = 2000
	ndim = 6
	Xs = rand(nobs, ndim) .* 2 .- 1
	# Xs_test = randn(nobs, ndim) .* range(1, 5, ndim)'
	# std(Xs, dims=1)
	ws = [1, 0, -3, 0, 5, 0] 
	bias = -4.
	σs = logistic.(Xs * ws .+ bias)
	ys = rand(nobs) .< σs
	using Distributions
	ys = rand.(Bernoulli.(σs))
...
# ╔═╡ 0d895ae1-11b2-4195-82f0-480d1a74b900
mean(ys)

# ╔═╡ 8fba60de-feed-4914-b6b2-e3c11241dbfa
# extrema(Xs, dims=1);

# ╔═╡ 3ff1e1a3-ee0d-4107-a81f-545f8880e6d4
begin
	# Xs_dummy = [ones(length(ys)) Xs]
	mus  = mean(Xs, dims=1)
	sigmas = std(Xs, dims = 1)
	Xs_std = (Xs .- mus) ./ sigmas
	Xs_std_dummy = [ones(length(ys)) Xs_std]
	Xs_dummy = [ones(length(ys)) Xs]
end;

# ╔═╡ 154dd63c-0250-4268-bc58-fb75ed0dcddf
# function sigmoid(z)
#     return 1/(1+exp(-z))
# end;

# ╔═╡ 23dd6e76-fd49-41bc-b54b-dc66f0b5fefa
# md"""
...
# ╔═╡ a40dae59-36c9-454d-87d7-346587806433
...
# ╔═╡ ae39d3b1-2ea9-4fee-8064-fc8461b0182b
function gradient_ce_loss(w, X, y)
    y_preds = logistic.(X * w)
	error = y_preds - y
	grad = (X' * error) /length(y)
    return grad
end;

# ╔═╡ bea81874-62a8-4461-9098-1ea4c8b47728
md"""
...
# ╔═╡ f09a53a8-2ead-4479-9e9d-b3305fbd564a
let
	## gradient test
	w0 = zeros(ndim+1)
	gradw_finitediff = FiniteDifferences.grad(central_fdm(5, 1), w -> cross_entropy_loss(w, Xs_dummy, ys), w0)[1]
	gradw_my_impl = gradient_ce_loss(w0, Xs_dummy, ys)
	gradw_finitediff ≈ gradw_my_impl
end

# ╔═╡ 8e566a60-8213-452d-bb35-87b7a66a795b
let
	## gradient test
	w0 = zeros(ndim+1)
	gradw_finitediff = FiniteDifferences.grad(central_fdm(5, 1), w -> cross_entropy_loss(w, Xs_std_dummy, ys), w0)[1]
	gradw_my_impl = gradient_ce_loss(w0, Xs_std_dummy, ys)
	gradw_finitediff ≈ gradw_my_impl
end

# ╔═╡ d1e89204-c8c3-409d-96d0-e5c1b8d222bb
...
# ╔═╡ 103566bb-f944-4549-9934-db6ea5d4954b
w_learnt, losses = logistic_regression_train(Xs_dummy, ys, 0.1)

# ╔═╡ 22444133-aae5-4867-bf1e-e7169c1ce17a
w_learnt_, losses_ = logistic_regression_train(Xs_std_dummy, ys, 0.1)

# ╔═╡ 6826ee1e-609d-4c73-9f79-df8799e91d6e
function predict_logistic_regression(w, X = Xs_dummy)
    n, d = size(X)
...
# ╔═╡ 5a6646d3-26c6-48c6-baf4-5587a68331f6
function accuracy(w, Xs, ys)
	ŷs = predict_logistic_regression(w, Xs)
	mean(ys .== ŷs)
end

# ╔═╡ aeffb20c-dad1-4649-9dd7-336dde31573a
predict_logistic_regression(w_learnt, Xs_dummy)

# ╔═╡ 3419a85d-0d7a-4438-b132-cef3a5c293c1
accuracy(w_learnt, Xs_dummy, ys)

# ╔═╡ 8464ea4e-93ff-4c5a-9229-6e69a110fbc3
accuracy(w_learnt_, Xs_std_dummy, ys)

# ╔═╡ 4aaffca7-98ee-4e52-8bd8-2e8396919b79
md"""
...
# ╔═╡ f16c9a3d-0a22-4afc-8acb-e1d7d9d1274a
begin
	## run your algorithm and report your findings
	w_reg_learnt, losses_reg = logistic_regression_reg_train(Xs_std_dummy, ys, 0.1, 0.001)
end

# ╔═╡ 448dc359-8abf-4614-9e1a-2c417f5fe92b
accuracy(w_reg_learnt, Xs_std_dummy, ys)

# ╔═╡ 272b5a6c-a0e8-4f8e-8e38-7e2a158ee501
w_learnt_
...
# ╔═╡ da499f8d-e4ee-4115-b763-56f0bdf0823e
begin
	## run your algorithm and report your findings
end
...
manifest_format = "2.0"
project_hash = "be10ed52b0b9c6a307f8da66a31264e303514e3f"
