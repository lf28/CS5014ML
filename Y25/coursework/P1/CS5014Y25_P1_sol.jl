...
begin	
	...
	data1url = "https://leo.host.cs.st-andrews.ac.uk/CS5014Y25/P1/datasets/dataset1.csv"
	dataset1 = Matrix(CSV.File(download(data1url)) |> DataFrame)
	d1X, d1Y = dataset1[:, 1:200], dataset1[:, end]
end;

# ╔═╡ a2861af0-5179-487c-b972-53d9084ba24a
...
## Question 1 (Lasso linear regression)
...
* and the last column is the regression targets ${y}^{(i)}$
...
# ╔═╡ eefacc58-80ff-43df-ad7b-506b8c077637
md"""
...
3. *soft-thresholding, set a small constant e.g. $\epsilon = 10^{-5}$, if the absolute value of any weight drops below $\epsilon$, set it to zero directly*
*You can do either 2 or 3 (or both).*
"""

# ╔═╡ 537f828f-af41-420f-abf9-fb2dcab0f91e
## implement your algorithm here

# ╔═╡ 52a1984c-7ca6-4beb-847e-f95b839aa7f5
md"""
...
# ╔═╡ a3516f5e-1368-44f0-bfdf-c7ebc6c24841
### report your results
...
begin	
	# download and read in dataset2
	data2url = "https://leo.host.cs.st-andrews.ac.uk/CS5014Y25/P1/datasets/dataset2.csv"
	...
	d2X, d2Y = dataset2[:, 1:4], Bool.(dataset2[:, end])
	d2_xtrain, d2_ytrain = d2X[1:1500, :], d2Y[1:1500]
	# the testing dataset has the last 500
	d2_xtest, d2_ytest = d2X[1501:end, :], d2Y[1501:end]
end;
...
# ╔═╡ 5ec5827c-d633-42f9-8ea2-fd101ef13a0d
sigmoid_(z) = (tanh(0.5 * z) + 1)/2

# ╔═╡ e0b3f824-e752-410c-972d-1d36638b58c4
# begin
# 	xx_ = randn()
# 	sigmoid_(xx_) ≈ sigmoid(xx_)
# end
...
function cross_entropy_loss(w, b, X, y)
	n = length(y)
    σs = sigmoid.(X * w .+ b)
	loss = -sum(log.(σs)[y .== one(eltype(y))])
	loss -= sum(log1p.(-σs)[y .== zero(eltype(y))])
    return loss/n
end;

# ╔═╡ e85424a1-d661-48f7-920e-e4cf71c67241
function cross_entropy_loss_(w, b, X, y)
	n = length(y)
    σs = sigmoid.(X * w .+ b)
	loss = - xlogy.(y, σs) - xlog1py.(one(eltype(y)) .- y, -σs)
    return mean(loss)
end;

# ╔═╡ 642edfa0-e556-40f0-9302-05fd2013e3de
begin

	ww_ = randn(4)
	cross_entropy_loss(ww_, 0., d2_xtrain, d2_ytrain), cross_entropy_loss_(ww_, 0., d2_xtrain, d2_ytrain)
end

# ╔═╡ 3e264881-9b86-4d2f-a0c7-085f711d1acc
function gradient_ce_loss(w, b, X, y)
    n = length(y)
    σs = sigmoid.(X * w .+ b)
	error = y - σs
	gW = - X' * error
...
# ╔═╡ cc9493fd-40f4-415c-a666-2920fce5593c
begin
	bb_ = 0.
	gradient_ce_loss(ww_, bb_, d2_xtrain, d2_ytrain)
end

# ╔═╡ 336bbe4c-ffbd-4eba-b1a6-59bbd8cf9260
FiniteDifferences.grad(central_fdm(5, 1), wb -> cross_entropy_loss(wb[1:end-1], wb[end], d2_xtrain, d2_ytrain), [ww_..., bb_])[1] 

# ╔═╡ 47e12025-2fc4-423a-b6b8-32d48244810f
md"""
Now, implement the gradient descent algorithm below. Before that, you should consider testing our gradient implementation with the finite difference method.
"""
...
function logistic_regression_train(X, y, lr, tol= 1e-5, maxIters= 2000)
    ...
    w0 = zeros(d)
	b0 = 0.0
    l0 = cross_entropy_loss(w0, b0, X, y)
    losses = [l0]
    # loop until converge
    for i in 1:maxIters
        ## Implement gradient descent here
        dw, db = gradient_ce_loss(w0, b0, X, y)
		w0 = w0 - lr * dw
		b0 = b0 - lr * db
        # Check convergence here 
        l0 = cross_entropy_loss(w0, b0, X, y)
		# if converge
		if abs(l0 - losses[end]) < tol
			break
		end
		push!(losses, l0)
        #    break
...
begin
	...
	w_learnt, b_learnt, losses_gd =logistic_regression_train(d2_xtrain, d2_ytrain, 0.1)
end

# ╔═╡ 456b2b5a-798d-4fcc-807b-b552b56885be
plot(losses_gd, xlabel="Epoch", ylabel="Loss")

# ╔═╡ eca3b50c-c2af-452e-be8f-f86b5afb62b3
begin	
	y_preds_test = sigmoid.(d2_xtest * w_learnt .+ b_learnt) .> 0.5
	mean(y_preds_test .== d2_ytest)
end
...
$$L(\mathbf{w}, b)=  -\frac{1}{n}\sum_{i=1}^n \left ( {y^{(i)}} \ln \sigma^{(i)}+ (1- y^{(i)}) \ln (1-\sigma^{(i)}) \right )+ \frac{\lambda}{2} \|\mathbf{w}\|_2^2$$
...
function logistic_regression_reg_train(X, y, lr, lam = 1.0, tol= 1e-5, maxIters= 2000)
    ...
    w0 = zeros(d)
	b0 = 0.0
    l0 = cross_entropy_loss(w0, b0, X, y) + 0.5 * lam * w0'*w0
    losses = [l0]
    # loop until converge
    for i in 1:maxIters
        ## Implement gradient descent here
...
begin
	## run your algorithm and report your findings
	w_ridge, b_ridge, losses_ridge = logistic_regression_reg_train(d2_xtrain, d2_ytrain, 0.1, 0.01)
end

# ╔═╡ 509d6852-9781-47ee-9643-bfc52346478c
plot(losses_ridge)

# ╔═╡ 5abf7739-42a3-4dc7-8ac8-6b3a9fc4c97d
md"""
...
# ╔═╡ 3aa36d37-7ae7-45a1-a4b2-9a70d92eff09
## Newton's method
function logistic_reg_newton(X, y, lam = 0.01, tol= 1e-5, maxIters= 2000)
	n, d = size(X)
    # initialise w, b
	ws = zeros(d+1)
	losses = []
	for i in 1:maxIters
		loss, gw = Zygote.withgradient(ws) do wb
			w, b = wb[2:end], wb[1]
			loss = cross_entropy_loss(w, b, d2_xtrain, d2_ytrain)
			loss + 0.5 * lam * w' * w
		end
		H = Zygote.hessian(ws) do wb
			w, b = wb[2:end], wb[1]
			loss = cross_entropy_loss(w, b, d2_xtrain, d2_ytrain)
			loss + 0.5 * lam * w' * w
		end
...
# ╔═╡ d80bcd9c-c01d-495d-b263-5b7d820a0f5e
begin
	lam = 0.01

	w0 = randn(5)
	loss, gw = Zygote.withgradient(w0) do ws
		w, b = ws[2:end], ws[1]
		loss = cross_entropy_loss(w, b, d2_xtrain, d2_ytrain)
		loss + 0.5 * lam * w' * w
	end

	H = Zygote.hessian(w0) do ws
		w, b = ws[2:end], ws[1]
		loss = cross_entropy_loss(w, b, d2_xtrain, d2_ytrain)
		loss + 0.5 * lam * w' * w
	end

	# H = Zygote.hessian(ws -> cross_entropy_loss(ws[2:end], ws[1], d2_xtrain, d2_ytrain) + 0.5 * lam * norm(ws[2:end])^2, w0)

	d = H \ gw[1]
	
end

# ╔═╡ 52d713c5-dae4-4462-845f-e65083614cbb
...
* implement a training algorithm of your choice to learn the parameter

* report the learnt parameter with $\lambda =0.1$
"""

# ╔═╡ f72c73ab-7079-41ba-abe7-be27a2d1806b
## write down the gradient expression
...
manifest_format = "2.0"
project_hash = "f41e81869c757c28eeb5ebbbfe7f8911a31bc3c7"
