...
# ╔═╡ d0202002-d637-483c-9bb0-71692f82591b
md"""

# One-versus-all scheme
"""

# ╔═╡ b226b7a8-da06-4461-815d-2eb14301ec95
md"""

## One-versus-all scheme

#### *Idea:* ``C``-class classification ``\Rightarrow`` ``C`` binary classifications 
"""

# ╔═╡ 0feeee43-6f50-4b5e-a010-ed6d99845365
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/threeclass.png
' width = '350' /></center>"

# ╔═╡ 532c1067-06a3-4ad9-a34c-b5d53d62be1c
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/onevsall1.png
' width = '750' /></center>"

# ╔═╡ 86522af8-1c17-462a-9cea-c4ae3d05cb49
md"""

## One-vs-all implementation


#### We need ``C`` logistic regressions with ``C`` outputs
\
...
# ╔═╡ b3c59824-68ab-4bcc-8550-eec18bed23dd
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/multiclass3.svg
' height = '350' /></center>"

# ╔═╡ 13cae6b5-641d-42d3-8898-8342121f6af3
md"""

## One-vs-all implementation


#### We need ``C`` logistic regression with outputs
\
...
* ##### then apply `logistic` function **individually** to squeeze output to ``(0,1)``

```math
\Large
\sigma_c =  \sigma(\mathbf{w}_c^\top \mathbf{x} + b_c)
```
"""

# ╔═╡ 43ec270f-80b5-42cc-8b7a-355fa0c8d41a
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/multiclass4.svg
' height = '350' /></center>"

# ╔═╡ d64b0167-95ac-4d18-b9f8-ec66230a1309
md"""
...
* #### ".": means apply the function element-wisedly
"""

# ╔═╡ 293118b6-3ac6-4739-b86f-201c66b1262b
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/multiclass5.svg
' width = '900' /></center>"

# ╔═╡ a15ba02d-bc0b-4ee5-a0e8-48601303509a
md"""
...
# ╔═╡ 11e93307-0f7d-4f9c-900e-87b2c43aa46b
md"""
#### Examples
"""

# ╔═╡ 31a30f40-ba71-4634-b6a1-b79b7dc46d0d
ThreeColumn(
md"""
##### label ``y=1``
...
# ╔═╡ b35a27cb-e872-4762-87ad-60053a1d9780
md"""
...
#### The loss for the ``i``-th observation is the sum of ``C`` binary cross entropy losses


```math
\Large 
L^{(i)}(\mathbf{W}, \mathbf{b}) = \sum_{c=1}^C \underbrace{-{{y}_c^{(i)}} \ln \sigma^{(i)}_c- (1- {y}^{(i)}_c) \ln (1-\sigma_c^{(i)})}_{\text{CE for class }c}
```

* ``y_c^{(i)} = \mathbb{1}(y^{(i)} = c)``: whether $y^{(i)}$ is class ``c`` or not

#### The total losses for the training dataset is 


```math
\Large 
\begin{align}
L(\mathbf{W}, \mathbf{b}) &= \frac{1}{n}\sum_{i=1}^n L^{(i)}(\mathbf{W}, \mathbf{b}) \\
&=\frac{1}{n}\sum_{i=1}^n\sum_{c=1}^C{-{{y}_c^{(i)}} \ln \sigma^{(i)}_c- (1- {y}^{(i)}_c) \ln (1-\sigma_c^{(i)})}
\end{align}
```
"""

# ╔═╡ aa31794e-504f-4add-bdf3-c0e769af8d35
md"""
## Gradient
...
# ╔═╡ ea0e6313-a89b-4919-87e8-8e6cc55c527b
md"""
...
\Large
\begin{align}
\hat{y} &= \arg\max_{c=1\ldots C} P({y} =c|\mathbf{W}, \mathbf{b})_{\text{one vs all}}\\
...
# ╔═╡ 6e3d80de-b9ee-4cfb-8e4a-de3e6b2d09c0
md"""

## Demonstration -- one vs all

#### We apply **mini-batch** `stochastic` gradient descent
* ##### SGD: more on this next week
"""

# ╔═╡ 8ff69200-e90a-4b12-97cb-61897872a2eb
md"""
## Demonstration -- one vs all

"""

# ╔═╡ dc498ead-de8a-4742-ac90-4b11271b8ca0
md"""
...
# ╔═╡ 73a1b0ea-57e9-47d2-ae9f-502a9aee2cec
...
# ╔═╡ a8dd4865-34f9-417d-b5b6-4493478ff4ea
...
# ╔═╡ b7da34cf-24da-4794-a1f7-ecff614e877e
md"""
...
", md"
``\text{sftmax}(\mathbf{z})=\frac{1}{.4+1.0+2.7}``$(latexify_md(round.(exp.([-1, 0, 1]);digits=1)))
...
## Softmax regression: 1 - dimensional
"""

# ╔═╡ f52b5248-c940-42f5-bdc3-7f7b9d8646ce
TwoColumn(md"""

#### Three linear functions 
* (aka affine transformation.)
```math
\large
\begin{cases}
z_1 = {w}_1 {x} + b_1\\
...
> ```
> * ##### ``y^{(i)}`` is a Categorical distributed with parameter $\boldsymbol\sigma^{(i)}$
"""
...
# ╔═╡ 6dc1bc25-17ca-4aee-8b8d-d0ffeee757c2
md"""
## Softmax regression vs One-vs-All

"""

# ╔═╡ 919d763c-5579-495e-bf96-265cc2020f73
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/multiclass5.svg
' width = '850' /></center>"

# ╔═╡ 3ca37bdd-2e37-442b-92d5-d7bd6b550e23
...
#### The negative log-likelihood is our loss (`cross entropy loss`)
...
# ╔═╡ bef90e36-f477-4fe3-bd55-dc93931b67ec
md"""
...
begin
	...
	function produce_plt(W0, iter, XX, loss)
		...
		xs = range((extrema(XX[:,1]) .+ [-1, 1])..., 100)
		ys = range((extrema(XX[:,2]) .+ [-1, 1])..., 100)
		plot!(xs, ys, (x, y) -> argmax(W0 * [1, x, y]), ratio=1, st=:heatmap, alpha=0.3, c=1:3,colorbar=false, title="Iteration $(iter); loss: $(round(loss; digits=2))", framestyle=:zerolines, xlims = extrema(XX[:, 1]) .+ [-1, 1], ylims = extrema(XX[:, 2]) .+ [-1, 1])
		return plt
	end
...
#### -- redefine the CE loss with **`logits`** ``\mathbf{z}`` instead of the softmax output ``\hat{\mathbf{y}}``
...
* ##### this is the preferred implementation of the loss in `PyTorch`, `Flux` and other ML packages
"""

# ╔═╡ fe0428fb-cad9-4be6-bc65-6ca920efefdd
show_img("ce_loss_forward2.svg", w=600)

# ╔═╡ e46f9b32-d027-4e4d-90bb-f88fdc6c775d
...
##### The partial derivative *w.r.t* the logits ``z_c`` is
...
#### The partial *w.r.t* the logits ``{z}_c`` is
```math
\large
...
##### We have the gradient of $\mathbf{w}_c$ 
...
#### Note that gradient for ``\mathbf{b}`` can be recovered by augmenting ``\mathbf{x}`` with dummy ``1``
...
* ##### it implies the gradient w.r.t the bias is just the prediction error
```math
\large
...
# ╔═╡ d335bca4-95b7-4358-801c-b50373d8b818
begin
	model2 = Chain(
	    Dense(imput_dim => output_class),  
	) |> f64       
...
# ╔═╡ 9c149745-d627-47c4-9769-7d95eb14d941
let
...
# ╔═╡ 11481043-275e-4c82-8215-0b15fe05e94b
TwoColumn(md"""
#### Softmax regression
...
# ╔═╡ 1f0d5bd1-45c3-4b66-addc-35d65a00caa2
let
...
# ╔═╡ dbb882a6-15db-4de7-bcd8-680b4be9e766
let
...
manifest_format = "2.0"
project_hash = "fcbb3e6032e2e0d3c0594bd65fbcbd4c4b657ac4"
