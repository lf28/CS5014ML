...
#### The algorithm needs
  ...
  * ##### ``\colorbox{lightgreen}{$\gamma$}``, a small positive _learning rate_ *e.g.* 0.01
...
!!! note ""
	...
	* ##### it implies it will converge (even left running forever) provided
	  ...
	  * ##### _and the learning rate is small_
 ...
 * ##### therefore, as $t\rightarrow \infty$, ``\large\mathbf{w}_t \leftarrow \underbrace{\mathbf{w}_{t-1} - \gamma \underbrace{\nabla f(\mathbf{w}_{t-1})}_{\rightarrow\mathbf{0}}}_{\mathbf{w}_{t-1}} ``
"""
...
#### In practice, we use the `central` approximation
...
* ##### this is very inefficient for large $\mathbf{w}$
* ##### we don't use finite difference method in gradient descent, only for checking!
"""
...
----
``\large\mathbf{w} = \mathbf{0}``

##### while not converge
* ##### ``\mathbf{w} \leftarrow \mathbf{w} - \gamma \cdot \mathbf{X}^\top(\mathbf{Xw} -\mathbf{y})``
...
#### Let's inflate second feature ``\times 10``

```math
\Large
\mathbf{x}_2 \leftarrow 10 \times \mathbf{x}_2 
```

* ##### scale the second feature (second column) by 10
...
$(latexify_md(round.(x_train_new[1:10, :], digits=2)))
""",
...
```math
\large
l(\mathbf{w})= \frac{1}{2n}\sum_{i=1}^n (y^{(i)} - \mathbf{w}^\top\mathbf{x}^{(i)})^2 
```
...
# ╔═╡ 7aa4d08e-cd96-451d-a3f5-54cab351c441
md"""

## Gabor regression -- implementation in Julia*
"""
...
function train(max_iters; w_init=zeros(2), γ = 0.1, x_train=x_train_gab, y_train=y_train_gab)
...
manifest_format = "2.0"
project_hash = "59370f509587965ccb8b379fc50a29437f0df755"
