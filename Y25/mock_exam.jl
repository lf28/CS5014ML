...
###### Consider a ``\mathbb{R}^2 \rightarrow \mathbb{R}`` function ``h(\mathbf{x}) = \mathbf{w}^\top\mathbf{x} + w_0``, where ``\mathbf{w} = [1, 2]^\top`` is known. Assume we are at $\mathbf{x}_0 = [1,1]^\top$;
...
$$\mathcal{L}(\mathbf{w}) = \frac{1}{n} \sum_{i=1}^n \ell^{(i)}(\hat{y}^{(i)}, {y}^{(i)}) + \frac{\lambda_1}{2} \|\mathbf{w}\|_2^2 + \lambda_2 \sum_j |w_j|,$$

where $\ell^{(i)}$ is the $i$-th observation's loss, $\hat{y}^{(i)} = h(\mathbf{w}, \mathbf{x}^{(i)})$ is the supervised learning's model output


* Outline a learning algorithm to train the model 
