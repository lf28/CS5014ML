...
# ╔═╡ 5d0d4256-0dbf-4b93-af1e-f82f850dcc8f
begin
	joint_P = [1 4 3 5 2; 1 2 6 2 1; 1 2 3 4 5]
	joint_P = joint_P // sum(joint_P) 
end;

# ╔═╡ 5567620b-fda0-4751-89bc-4c7183feef1e
md"""
## Joint distribution example
"""

# ╔═╡ 4ef4fed7-cee6-4fb6-b7c0-857ed614ebe9
md"""


```math
\Large
\begin{equation}  \begin{array}{c|ccccccc} 
P(X, Y) & Y = 1 & Y = 2 & Y = 3 & Y = 4 & Y = 5   \\
\hline
X=1 & \frac{1}{42} & \frac{2}{21} & \frac{1}{14} & \frac{5}{42} & \frac{1}{21} \\ X=2 & \frac{1}{42} & \frac{1}{21} & \frac{1}{7} & \frac{1}{21} & \frac{1}{42} \\ 

X=3 & \frac{1}{42} & \frac{1}{21} & \frac{1}{14} & \frac{2}{21} & \frac{5}{42} \\  \end{array} \end{equation} 

```

"""

# ╔═╡ cfc2e11a-3cdd-4585-aa67-6201433f6186
md"""Hightlight ``y``: $(@bind hightlight_y CheckBox(false)), Hightlight ``x``: $(@bind hightlight_x CheckBox(false))"""

# ╔═╡ 61cb229d-7611-42d7-9d9c-e8519fcae215
joint_plt = let
	gr()
	nx, ny = size(joint_P)
	xtsize, ytsize = 15, 15
	...
	if hightlight_y
 	...
 	plt = plot(xlim=[0.5,ny +0.5], ylim=[0.6, nx+0.5], xticks=1:ny, yticks =1:nx, zlim = [0, maximum(joint_P)+0.05], zaxis=true, framestyle=:semi, camera=(10,25), size=(800, 600), xlabel=L"y", ylabel=L"x", zlabel=L"P(x,y)", title=L"P(X,Y)", xlabelfontsize=xlsize, ylabelfontsize =ylsize, titlefontsize=20, xtickfontsize=xtsize, ytickfontsize=ytsize)
	for i in 1:size(joint_P)[1]
		for j in 1:size(joint_P)[2]
			col = hightlight_x ? my_colors[i] : my_colors[j]
			quiver!([j], [i], [0], quiver=([0], [0], [joint_P[i,j]]), lc = col, lw=5)
		end
	end
	plt
end;

# ╔═╡ 620fd832-b6f2-4c0d-9e03-d9c74d5db07e
joint_plt

# ╔═╡ 38c0cd6f-85f9-41a8-a5f3-a5eba3b428d7
md"""L"$(begin latexify(joint_P) end)""";

# ╔═╡ e91acc62-1de3-4f35-b5ff-35358053b4bd
marg_py= let
	bar(sum(joint_P, dims=1)[:], xticks=1:size(joint_P)[2], c= 1:size(joint_P)[2],title=L"P(Y)", label="")
end;

# ╔═╡ 67828592-4825-4f5c-8f00-dfde6fbc3a51
marg_px = let
	bar(sum(joint_P, dims=2)[:], yticks=1:size(joint_P)[1], orientation=:h, c= 1:size(joint_P)[1] ,yflip=true, label="", title=L"P(X)")
end;

# ╔═╡ d5034885-dee1-4f4d-93b1-37336c3e6380
md"""Hightlight ``x``: $(@bind hightlight_x_ CheckBox(false))"""

# ╔═╡ 108c8f14-5d7d-4476-9ece-076b072527cb
md"""``Y=``$(@bind yvalue Slider(1:size(joint_P)[2])), ``X=`` $(@bind xvalue Slider(1:size(joint_P)[1]))"""

# ╔═╡ 5a041fa1-e1f9-4e21-97f9-81575b9cbb05
let
	...
	my_colors = 1:(nx* ny)
 	...
 	joint_plt = plot(xlim=[0.5,ny +0.5], ylim=[0.6, nx+0.5], xticks=1:ny, yticks =1:nx, zlim = [0, maximum(joint_P)+0.05], zaxis=true, framestyle=:semi, camera=(10,25), size=(700, 700), xlabel=L"y", ylabel=L"x", zlabel=L"P(x,y)", title="", xlabelfontsize=xlsize, ylabelfontsize =ylsize, titlefontsize=20, xtickfontsize=xtsize, ytickfontsize=ytsize)
	for i in 1:size(joint_P)[1]
		for j in 1:size(joint_P)[2]
			col = hightlight_x_ ? my_colors[i] : my_colors[j]
			lw = yvalue == j ? 9 : 5
			if hightlight_x_
				lw = xvalue == i ? 9 : 5
			end
			quiver!([j], [i], [0], quiver=([0], [0], [joint_P[i,j]]), lc = col, lw=lw)
		end
	end
	...
	if hightlight_x_
		...
		alphas = 0.5 * ones(nx)
		alphas[xvalue] = 1.0
		marg_py = bar(sum(joint_P, dims=1)[:], xticks=1:size(joint_P)[2], ylim =(0,0.4), title=L"P(Y)", label="", xlabel=L"Y", bar_width = 0.5)
		marg_px = bar(sum(joint_P, dims=2)[:], yticks=1:size(joint_P)[1], orientation=:h, c= 1:size(joint_P)[1] ,yflip=false, xlim =(0, 0.6), label="", title=L"P(X)", ylabel=L"X", bar_width =bws, alpha = alphas)
	else
		bws = 0.5 * ones(ny)
		bws[yvalue] = 0.8
		alphas = 0.5 * ones(ny)
		alphas[yvalue] = 1.0
		marg_py = bar(sum(joint_P, dims=1)[:], xticks=1:size(joint_P)[2], c= 1:size(joint_P)[2],title=L"P(Y)", label="", alpha =alphas, ylim =(0,0.4), xlabel=L"Y", bar_width = bws)
...
manifest_format = "2.0"
project_hash = "8a676805b5a3a58c8c67df60ea1c5f06667b1856"
