...
#### Optimising/minimising an objective function

```math
\Large
\hat{\mathbf{w}}\leftarrow \arg\min_{\mathbf{w}} \ell(\mathbf{w})
```
...
```math
\Large
\boxed{\mathbf{w}^{(t)} \leftarrow \mathbf{w}^{(t-1)} - γ\, \mathbf{g}^{(t-1)}}
```

* ##### ``\gamma > 0``: learning rate

* ##### ``\mathbf{g}`` denotes the gradient, *i.e.*
...
#### The objective function: a _quadratic_ function
...
##### _However,_ the *Zig-zag* behaviour is stil not ideal
...
!!! question "Compute average on-line"
	...
	```
	* ###### we do not know the total sample size $n$ in advance
	

	#### How to compute the sample mean online?
	```math
	\Large
...
#### Then the sample mean at $t$: ``\mu^{(t)}``
...
# ╔═╡ bb436bf3-8452-4924-b08b-89d0a16acffb
md"""
...
#### We have found a _recursive_ update formula to compute ``\mu^{(t)}``

```math
\Large
\begin{align}
\mu^{(t)} &= \normalsize \frac{1}{t}\left[\;\overbrace{\underbrace{(t-1)\;\cdot\;\mu^{(t-1)}}_{d_1+d_2+\ldots+d_{t-1} }\;+\; d_t}^{\text{sum of first } t \text{ data}}\;\right ]\\
&= \boxed{\frac{t-1}{t}\, \mu^{(t-1)} + \frac{1}{t}\, d_t }
\end{align}
```
...
# ╔═╡ 7853721b-bba3-4758-b998-65e5213d7b2b
md"""

## Another example


#### The sample mean lags behind the trend
"""
...
&\approx 1 \cdot \mu^{(t-1)} + 0\cdot\, d_t\\

&= \mu^{(t-1)}
\end{align}
```

* ##### the mean ``\mu^{(t)}`` "converges"
...
# ╔═╡ dfcd4549-0530-4888-b16e-7d139f627d7d
plt_mean2
...
## Why it is called _exponentially weighted_?


#### It can be proved that the weights are **exponentially weighted**
...
#### A simple solution to resolve the zig-zag behaviour is: `Momentum`


* ##### the idea: use **the average of** some most recent **gradients**
...
#### Apply ``\texttt{sign}`` element-wisely to each ``\frac{\partial }{\partial w_j} \ell(\mathbf{w})``
...
* ##### apply ``\texttt{sign}`` element-wisely to the gradient
...
#### Therefore, `RProp` can also be formulated as 
...
\\ \vdots \\ \frac{\mathbf{g}^{(t-1)}_m}{ \sqrt{\left (\mathbf{g}^{(t-1)}_m \right )^2}} \end{bmatrix} \\
\mathbf{w}^{(t)} &\leftarrow \mathbf{w}^{(t-1)} - γ\, \mathbf{d}^{(t-1)}
\end{align}
```
"""
...
#### In practice, we add a small constant ``\epsilon`` to avoid dividing zero
...
\\ \vdots \\ \frac{\mathbf{g}^{(t-1)}_m}{ \sqrt{\left (\mathbf{g}_m^{(t-1)} \right )^2}+\epsilon} \end{bmatrix} \\
\mathbf{w}^{(t)} &\leftarrow \mathbf{w}^{(t-1)} - γ\, \mathbf{d}^{(t-1)}
\end{align}
```
...
#### `RProp` can be specified as 
* ##### (here we introduce ``\mathbf{v}``: gradient's square)
...
#### -- minibatch version of `RProp`
\


#### **R**oot **M**ean **S**quared **Prop**ogation (`RMSProp`) is a variant of `RProp`

* ##### stochastic gradient descent's gradient is noisy


* ##### use **exponentially weighted moving average** to smooth out the gradient square ``\mathbf{v}^{(t-1)}`` between the batches
...
#### **Ada**ptive **M**oment Estimation (`Adam`) is another variant 
...
# ╔═╡ ca67d229-b5e8-435d-b257-501cbd311bb4
aside(tip(md"""Adam optimiser in the original paper also has an optional bias correction step

$$\mathbf{m}^{(t-1)} \leftarrow \frac{\mathbf{m}^{(t-1)}}{1-\beta_1^{t-1}}$$


$$\mathbf{v}^{(t-1)} \leftarrow \frac{\mathbf{v}^{(t-1)}}{1-\beta_2^{t-1}}$$

* they correct the biases because of the initial 0 used for $\mathbf{m}, \mathbf{v}$
"""))
...
#### For most problems, _esp._ deep networks with noisy gradients 

* ##### `Adam` (and also `RMSProp`) works well straight out of the box
...
# ╔═╡ 77bbba4d-9a13-4d01-85db-1bed30094cae
# import Flux:Chain, Dense,relu

# ╔═╡ dcc15039-4383-4d10-a580-5a153a38121d
# deep_net = Chain(Dense(2 => 100, relu), Dense(100 => 100, relu), Dense(100 => 100, relu), Dense(100 => 100, relu), Dense(100 => 100, relu),  Dense(100 => 1))

# ╔═╡ 9b94c18d-8298-4ed4-9631-4cfedc874407
md"""

## Demonstration: image learning


#### Learn the same network: two optimisers with mini-batch

* ##### simple vanilla `stochastic gradient descent` (SGD)

* ##### and `Adam`
"""

# ╔═╡ 19d4c368-6575-4d4e-96c0-fc05a874833e
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/sgdvsadam.png
' width = '400' /></center>"

# ╔═╡ e155b5ea-b649-4e24-8962-a77f3332dc6f
html"<center><img src='https://leo.host.cs.st-andrews.ac.uk/figs/sgdvsadam.gif
' width = '600' /></center>"
...
```
#### or simply $(latexify_md(round.(gt; digits=2)))
...
plot(plts_g1..., layout=(2,2), titlefontsize=12, size=(800, 450))
...
manifest_format = "2.0"
project_hash = "89f21c305dd030b2a0291d7cc57f4ae585f6c300"
