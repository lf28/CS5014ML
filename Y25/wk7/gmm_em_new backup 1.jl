...
$$\Large\mathbb{E}[X] = \int_{0}^1 p(X=x) x dx =\int_{0}^1 x dx = \frac{1}{2}x^2\Big\vert_{0}^1=\frac{1}{2}$$
...
* it should definitely be greater than $0.5$!
"""
...
$$\Large\begin{align}\mathbb{E}[f(X)] &= \int_{0}^1 p(X=x) f(x) dx \\
...
$$\Large\mathcal{L}(\theta) =\ln p(X |\theta) = \ln \int_Z p(X, Z|\theta) dZ\;\;\;\;\; \text{hard to compute!}$$
...
manifest_format = "2.0"
project_hash = "ae5180e19036b065a59f84473ef17932f6a28f03"
