...
And further updates have no effect:  since the gradient will be zero and 
...
$$\mathbf{w}_t \leftarrow \mathbf{w}_{t-1} - \gamma \cdot (\nabla \ell(\mathbf{w}_{t-1}) + \lambda_1 \mathbf{w}_{t-1} + \lambda_2 \texttt{sign}(\mathbf{w}_{t-1})))$$
