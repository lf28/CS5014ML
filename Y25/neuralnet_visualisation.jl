...
# ╔═╡ fe995e99-2946-434d-aff5-0ceb22770a15
begin
	using LinearAlgebra
	using PlutoUI
...
# ╔═╡ 2778c6fa-c135-11ee-1867-afd30f5df0d6
using MLJBase

# ╔═╡ d62b30cb-1cef-4d23-9675-19c4ecd60ff2
begin
...
# ╔═╡ a97b0ab6-3b66-4729-842c-1c6c489641a8
begin
	nobs_moon = 200
...
# ╔═╡ 76009f52-ff5a-448a-9c7e-d8fdfc9deb73
md"Add vectors: $(@bind add_vecs CheckBox(default=false))"

# ╔═╡ 08cb8047-6b77-4cd3-8c57-f96fed7459d3
colorch = cgrad(:hot, rev=true, scale =:linear);

# ╔═╡ fbaf9426-abd8-4c27-8660-a57622df5f0f
@bind δ_lt Slider(-2:0.1:2)

# ╔═╡ 4ac439c1-bf44-49f9-bf64-f7bf90eb4181
A_sheer_lr = [1 δ_lt; 0 1]

# ╔═╡ bc4ed141-add9-4a35-bee2-4176502cba30
let
	A_mat = A_sheer_lr
...
# ╔═╡ 4fdf89e0-4435-4198-8689-de2a03c73e27
begin
	nobs_circles = 500
	X_circle, y_circle = MLJBase.make_circles(nobs_circles; noise=0.05)
	# Matrix(X_moon)
	X_circle = matrix(X_circle)
	y_circle = Array(y_circle)
end;

# ╔═╡ 16b9b2c3-de86-4e2a-b232-772e44bc1534
rbf(x, μ, s) = exp(-0.5 * (x - μ)' * ((x-μ) ./s) )

# ╔═╡ ff346860-4c5c-41a5-afc0-d449fe6017b2
rbf([1,1], [0,0], 1)

# ╔═╡ 85440738-b2da-4f3b-8a0b-ed005f4c642b
f_rbf(x) = [rbf(x , [0,0], 0.1), rbf(x, [0,0], 0.2)]

# ╔═╡ 93c201bb-df5c-4690-959d-0cc15843e3b1
let
	A_mat = I
	plt = plot(ratio=1, xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines)
...
# ╔═╡ 5dea6a7f-73b5-48bb-9fcc-d17239af474e
f(v) = [v[1]+v[2], sin(v[1]) * cos(v[2]) + v[2]]

# ╔═╡ 58bf02f4-f41e-4815-8cc0-8fdaf2dd2979
f_quadratic(x) = [x[1]^2+x[2]^2,  x[1]*x[2]]

# ╔═╡ 9419b0a2-46bb-447a-9818-082730788123
let
	f = f_quadratic
	# A_mat = A_sheer_lr
	plt = plot(ratio=1, xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines)
...
# ╔═╡ 5822a3d6-5be3-4bc4-828d-188a8a8cea02
# let
# 	# xys_ = xys * A_mat';
# 	gr()
# 	f = f_quadratic
...
# 	plt = plot(xys_[:, 1], xys_[:, 2], st=:scatter, ratio=1, marker_z=zs, c=1, colorbar=:false, framestyle=:origin, markersize=4, alpha=0.5, label="", xlim = extrema(xys_[:, 1]) .+ (-1, 1), ylim =extrema(xys_[:, 2]) .+ (-1, 1))
...
# ╔═╡ 43c46172-18fc-40be-af14-962b4d933111
Flux.onehotbatch(y_moon, [true, false]) 

# ╔═╡ 3d1345e5-71a0-4c9f-b5d0-f3f7e39baff3
begin
	...
	moon_loader = Flux.DataLoader((X_moon', y_moon'), batchsize=100, shuffle=true);
...
# ╔═╡ 218132b1-3e47-4d2f-a284-7c7802220dbb
# plot(losses)

# ╔═╡ 4e96ae89-2b00-423d-bd7a-7c186ab15042
moon_loader |> first

# ╔═╡ a2fff37e-80fa-4525-9d37-c664676277a6
nnet_moon(X_moon')

# ╔═╡ f6a87cb6-0d52-47e2-a0a1-86441220b257
nnet_moon[end](X_moon')

# ╔═╡ ca007476-eec0-451b-8fad-4126a1b1236c
plts = let
	plts = []
	xnew = X_moon'
	plt = plot(ratio=1, title="Layer 0", framestyle=:zerolines)
	...
	for (i, f) in enumerate(nnet_moon[1:(end-1)])
		xnew = f(xnew)
		plt = plot(ratio=1, title="Layer $(i)", framestyle=:zerolines)
	...
	# xnew = nnet_moon[1](X_moon')

	# plt2 = plot(ratio=1)

	# for c in [0, 1]
	# 	scatter!(xnew[1, y_moon .== c], xnew[2, y_moon .== c], c= c+1, label="class $(c)")
	# end
	# plt
	# plt
	plts
...
# ╔═╡ b761c265-65ff-49da-bee7-3f01e28bc825
plot(plts..., titlefontsize=12)

# ╔═╡ 02fc0411-ed80-44ce-889e-337d4633c214
nnet_moon[3]

# ╔═╡ d603462a-2ff1-4b4a-8093-7e79ea35aebc
meshgrid(x, y) = (repeat(x, outer=length(y)), repeat(y, inner=length(x))) # helper function to create a quiver grid.

# ╔═╡ 8faf996a-624c-40cb-aeca-9e03ed1623e8
let
	gr()
	max_lim = 5
	xs = range(-5, 5, 10)
...
# ╔═╡ a1a97a88-961e-4cb7-95e3-bc0e1d9e12c3
begin
	max_lim = 5
	xs = range(-1, 1, 20)
	ys = range(-1, 1, 20)
...
# ╔═╡ 1547b178-406b-4719-bc38-5722b9204c2d
...
# ╔═╡ f7c15b7f-87bf-475b-b906-4c898a0d0b38
zs = (xys[:, 1])[:] ;

# ╔═╡ 007b363b-3fed-4008-b79b-5b878ab9cd06
let
	gr()
...
# ╔═╡ 96643939-1973-4c0f-91c3-8d43aba79247
let
	gr()
	A_mat = A_sheer_lr
	xys_ = xys * A_mat';
...
# ╔═╡ 52d013f1-35f3-4e07-8260-6454bc2ee3e0
let
	...
	# plt = plot(xys_[:, 1], xys_[:, 2], st=:scatter, ratio=1, marker_z=zs, c=1, colorbar=:false, framestyle=:origin, markersize=4, alpha=0.5, label="", xlim = extrema(xys_[:, 1]) .+ (-1, 1), ylim =extrema(xys_[:, 2]) .+ (-1, 1))
	plt = plot(ratio=1, framestyle=:zerolines)
...
# ╔═╡ b8542c84-ffce-4c3f-9163-26944c0be4e6
y_circle

# ╔═╡ a4c5a12d-a055-441e-8e11-05e334b6e194
begin
	...
	act = gelu
	nnet_circle = Chain(
	    Dense(2 => 2, act),   # activation function inside layer
	...
	# y_circle_one_hot = Flux.onehotbatch(y_circle, [true, false]) 
	# moon_loader = Flux.DataLoader((X_moon', y_moon_one_hot), batchsize=100, shuffle=true);
	circle_loader = Flux.DataLoader((X_circle', y_circle'), batchsize=250, shuffle=true);

	optim_cir = Flux.setup(Flux.Adam(0.01), nnet_circle)  # will store optimiser momentum, etc.

# # Train
	losses_circ = []
	for i in 1:10_00
		for (xtrain, ytrain) in circle_loader
	 		loss, grad = Flux.withgradient(nnet_circle) do m
				y_preds = m(xtrain) 
				# Flux.Losses.logitcrossentropy(y_preds, ytrain)
				Flux.Losses.logitbinarycrossentropy(y_preds, ytrain)
			end
			Flux.update!(optim_cir, nnet_circle, grad[1])
			push!(losses_circ, loss)
		end
...
# ╔═╡ 00dd5aed-495d-423a-b39f-8557a35fe745
plot(losses_circ)

# ╔═╡ 7dd1b369-4bcb-4787-ac45-54bd9438c2e0
plts2 = let
	plts = []
	xnew = X_circle'
	plt = plot(ratio=1, title="Layer 0", framestyle=:zerolines)

	xs = range(extrema(X_circle[:, 1]) .+ (-.5, .5)..., 10)
	ys = range(extrema(X_circle[:, 2]) .+ (-.5, .5)..., 10)
	# xs_ = range(extrema(xnew)..., 100)
	row_lines = []
	for y in ys
		xs_ = range(extrema(X_circle[:, 1]) .+ (-.5, .5)..., 50)
		ps = [xs_ ones(length(xs_)) * y]
		ps_ = ps
		plot!(ps_[:, 1], ps_[:,2], st=:path, alpha=0.5,  lc=:gray,label="")
		push!(row_lines, ps_)
	...
	col_lines = []
	for x in xs
		ys_ =  range(extrema(X_circle[:, 2]) .+ (-.5, .5)..., 50)
		ps = [ones(length(ys_))*x ys_]
		ps_ = ps
		plot!(ps_[:, 1], ps_[:,2], st=:path, alpha=0.5,  lc=:gray,label="")
		push!(col_lines, ps_)
	...
	for c in [0, 1]
		scatter!(xnew[1, y_circle .== c], xnew[2, y_circle .== c], c= c+1, label="class $(c)", alpha=0.5)
	end

	push!(plts, plt)
	for (i, f) in enumerate(nnet_circle[1:(end-1)])
		xnew = f(xnew)
		plt = plot(ratio=1, title="Layer $(i)", framestyle=:zerolines)
		for (ri, row) in enumerate(row_lines)
			# ps = [xs ones(length(xs)) * y]
...
# ╔═╡ 06b1de91-ba51-4d49-9a21-68177e89284d
plts2[end]

# ╔═╡ d95c385d-2a9d-48b1-8524-cf77d4112f77
begin
	rr = 1.0
	X_circle_2 = rand(2, 3000) .* 4 .- 2 
 	# y_circle_2 = zeros(Bool, size(X_circle_2))

	y_circle_2 = (sqrt.(sum(X_circle_2.^2, dims=1))[:] .< rr)
	plot(X_circle_2[1, y_circle_2], X_circle_2[2, y_circle_2], st=:scatter, ratio=1)
	plot!(X_circle_2[1, .!y_circle_2], X_circle_2[2, .!y_circle_2], st=:scatter, ratio=1)

end

# ╔═╡ 2d77fd03-3932-45e4-aecf-505fef0ab48f
X_circle_2, y_circle_2

# ╔═╡ 49831b8a-41a8-4177-9c7e-b1611f85db87
begin

	Random.seed!(1234)

	nnet_circle2 = Chain(
	    Dense(2 => 10, tanh; init = Flux.randn),   # activation function inside layer
		Dense(10 => 1)
	) 

	# y_circle_one_hot = Flux.onehotbatch(y_circle, [true, false]) 
	# moon_loader = Flux.DataLoader((X_moon', y_moon_one_hot), batchsize=100, shuffle=true);
	circle_loader2 = Flux.DataLoader((X_circle_2, y_circle_2'), batchsize=500, shuffle=true);

	optim_cir2 = Flux.setup(Flux.Adam(0.01), nnet_circle2)  # will store optimiser momentum, etc.

# # Train
	losses_circ2 = []
	for i in 1:20_00
		for (xtrain, ytrain) in circle_loader2
	 		loss, grad = Flux.withgradient(nnet_circle2) do m
				y_preds = m(xtrain) 
				# Flux.Losses.logitcrossentropy(y_preds, ytrain)
				Flux.Losses.logitbinarycrossentropy(y_preds, ytrain)
			end
			Flux.update!(optim_cir2, nnet_circle2, grad[1])
			push!(losses_circ2, loss)
		end
...
# ╔═╡ 62e5bdd3-29c6-461a-acb9-9946f4f5eeb5
plot(losses_circ2)

# ╔═╡ 76a61f6d-ee03-4684-bb58-e8ddf197440e
nnet_circle2[1].weight

# ╔═╡ 23627b42-7b8a-4970-8c79-61e03bc876f7
begin

	# plt = plot(ratio=1, xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines, xlim =[-2,2], ylim =[-2,2])

	plt = plot(X_circle_2[1, y_circle_2], X_circle_2[2, y_circle_2], st=:scatter, ratio=1, framestyle =:none, xlim =[-2,2], ylim =[-2,2], alpha=0.5, ms=3)
	plot!(X_circle_2[1, .!y_circle_2], X_circle_2[2, .!y_circle_2], st=:scatter, ratio=1, ms=2, alpha=0.2)
	ks = -nnet_circle2[1].weight[:, 1] ./ nnet_circle2[1].weight[:,2]
	bs = -nnet_circle2[1].bias ./ nnet_circle2[1].weight[:,2]


	# plt_circle = plot(ratio=1)

	for (k, b) in zip(ks, bs)
		plot!(-3:0.1:3, x -> k * x +b, lw=2, lc=:gray, label="")
	end

	plt
end

# ╔═╡ 08abcb8b-0ca2-4dd2-9641-6d794c645c16
ks
...
manifest_format = "2.0"
project_hash = "981a27209917dd948fd5012eb4e64e607764f3cc"
