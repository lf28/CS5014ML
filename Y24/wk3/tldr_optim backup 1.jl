...
# ╔═╡ e959b4a3-e235-4c67-a056-192d517245eb
md"""
...
#### A differentiable function ``f(x)``  can be approximated locally by a linear function
...
let
	...
	if add_quadratic2
		...
		plot!(p, range(x_center -1, x_center +1, 30), (x) -> taylor_approx(x; x̂=x̂, order=2); label=L"quadratic approx. at $x_0$", lc=3,  lw=3, title="Quadratic approximation")
		fpptxt = ""
		if fprimep > 0
			fpptxt = Plots.text(L"f^{''}(x)>0", :green)
...
#### The same applies to multivariate function
* ##### a differentiable function approximated by a hyper-plane
"""
...
# ╔═╡ 60d434a9-704a-4c8d-a883-4e0ff585691b
...
# ╔═╡ ef026576-5697-4cd6-b95e-065be23e4920
...
begin
	plt1 = plot(x -> x^2, xlim =[-3,3], ylim =[-1, 3], title=L"a>0",c=1, label="")
	plt2 = plot(x -> -x^2, xlim =[-3,3], ylim =[-3, 1], title=L"a< 0", c=2,label="")
	plt3 = plot(x -> 0, xlim =[-3,3], ylim =[-1, 1], title =L"a=0", c=3, label="")
...
```math
\Large
ax^2 \Rightarrow f(\mathbf{x}) = \mathbf{x}^\top\mathbf{A}\mathbf{x}
```
"""
...
manifest_format = "2.0"
project_hash = "8ba98fb43926dfadb81e0c0ecae2e7f116bf3ccd"
