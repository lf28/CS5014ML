...
# ╔═╡ 7307a7bd-c931-4e44-9b22-35c01d38504e
using MLJBase

# ╔═╡ 9d9507b7-15a2-4cee-b540-44416c52fd8c
using Zygote

# ╔═╡ a5b0c07f-1bff-4ea6-95d8-5064322fd29c
using Distributions
...
# ╔═╡ c6a3e306-bf55-44d0-a0ba-9cb9598791a5
md"""
...
* **Fixed basis models** [_Pattern recognition and Machine Learning_ by _Christopher Bishop_: Chapter 3.1.1 to 3.1.3; 4.3.1 to 4.3.3](https://www.microsoft.com/en-us/research/uploads/prod/2006/01/Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf)



"""

# ╔═╡ 1b08572b-9f1a-4205-b09a-da30558ec165
md"""

## _Linear_ regression 
"""

# ╔═╡ 30a2ee77-e604-4c98-ba67-d8ad1aec80c6
TwoColumn(md"""
\
\
...
# ╔═╡ a5062267-f7bd-4d2e-9c04-8c6ed1417827
md"""

## Quadratic ``h(\mathbf{x})``



##### Quadratic ``h(x)`` fits the house data **better**
...
# ╔═╡ fd56555c-9dd9-4027-8564-6542a6703676
quadratic_fit = let
	gr()
...
# ╔═╡ 285e0780-fe1a-41f9-8987-6c8ab6fbc9f9
...
# ╔═╡ e68f9d0f-7daf-4956-83fa-f670e95d3746
function least_square_est(X, y) # implement the method here!
	X \ y
end;

# ╔═╡ 9436a4a3-b046-4763-b75e-df9fd94b080a
md"""
...
##### For each ``x``， *expand* feature ``\colorbox{salmon}{$x^2$}``
...
# ╔═╡ 9511b077-e22b-4000-b32b-f1d0fc99c47e
md"""
...
##### with weight ``\mathbf{w} =[w_0, w_1, w_2]^\top``, the _regression function_ becomes
...
# ╔═╡ cfe2d697-b516-4659-a117-8c1b17d477e4
md"""
...
# ╔═╡ d12ffcb1-0715-47f2-a7f7-b172e546daef
md"""
...
#### The *predictions* ``\hat{\mathbf{y}}`` are

```math
\Large
\hat{\mathbf{y}} = \tilde{\mathbf{X}} \mathbf{w} = \begin{bmatrix}w_0 + w_1 x^{(1)} + w_2(x^{(1)})^2 \\
...
#### Lastly, the least square method can be applied
...
# ╔═╡ 28f5ab80-e138-4f60-840d-b316fd9c1ddd
begin
	x_room = df_house[:, :rm]
...
# ╔═╡ 5206f2b0-dd65-4528-8faa-6cbfe71256ae
md"""
...
# ╔═╡ d82955da-69e9-45f1-97e8-02eba2716ce5
md"""
...
```
* ##### still _free lunch_: regress with a ``n\times \textcolor{red}{(p+1)}`` matrix

"""

# ╔═╡ bd224549-385e-4c37-81ba-8933fadd41f9
md"""
...
# ╔═╡ d4d9c89f-7f86-4317-864a-7fde622d94d0
md"""

## Demonstration
"""

# ╔═╡ 66ad0293-4231-4933-a29e-524c10acc0f7
md"Polynomial order: $(@bind poly_order Slider(0:10, default =2, show_value=true))"

# ╔═╡ f2ba616d-94a3-459e-a986-0d97068ea6c5
begin
	nobs_circles = 500
	X_circle, y_circle = MLJBase.make_circles(nobs_circles; noise=0.025)
	# Matrix(X_moon)
	X_circle = matrix(X_circle)
	y_circle = Array(y_circle)
end;

# ╔═╡ a61089cd-a95f-424c-8906-f66ca2240d23
md"""
...
# ╔═╡ ff596f2c-8cef-4121-b688-98dee9b705e5
let
	A_mat = I
	plt = plot(ratio=1, xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines, labelfontsize=18, legendfontsize=15)

	X_circle_new = X_circle * A_mat'
	for c in [0, 1]
		scatter!(X_circle_new[y_circle .== c, 1], X_circle_new[y_circle .== c, 2], c= c+1, label="class $(c)")
	end

	plt
end

# ╔═╡ f466459c-0661-4e19-aebc-3441f4b52575
TwoColumn(md"""

$\Large \begin{bmatrix}x_1\\ x_2\end{bmatrix} \rightarrow \begin{bmatrix}x_1^2\\ x_2^2\end{bmatrix}$
""", md"""
$\Large \begin{bmatrix}x_1\\ x_2\end{bmatrix} \rightarrow x_1^2+x_2^2$
""")

# ╔═╡ 2b62db8c-8100-4355-93ba-12ca85a37df0
f_quadratic(x) = [x[1]^2,  x[2]^2];

# ╔═╡ 573492ad-a3a4-4b8a-98d6-5fcaa97a0007
f_quadratic2(x) = [x[1]^2+x[2]^2,  0];

# ╔═╡ 8d2d946a-1163-46e1-9ff0-670dc4874c9b
meshgrid(x, y) = (repeat(x, outer=length(y)), repeat(y, inner=length(x))) ; # helper function to create a quiver grid.

# ╔═╡ 3165b74b-546e-41d6-b238-dcf540f6709d
begin
	max_lim = 5
	xs = range(-1, 1, 20)
	ys = range(-1, 1, 20)
...
# ╔═╡ 8e24e560-d699-4cb9-bf59-96c773cad8b7
plt_poly_class_2d = let
	gr()
	f = f_quadratic	
	xys_ = hcat(f.(eachrow(xys))...)'
	plt = plot(ratio=1.0, framestyle=:zerolines, xlabel=L"x_1^2", ylabel=L"x_2^2", labelfontsize=20, legendfontsize=12, legend=false)
	...
	for x in xs
		...
		X_circle_new = hcat(f.(eachrow(X_circle))...)'
	for c in [0, 1]
		scatter!(X_circle_new[y_circle .== c, 1], X_circle_new[y_circle .== c, 2], c= c+1, label="class $(c)")
	end
...
# ╔═╡ 0c5053b4-b7b1-4613-89c0-c98a9b99b8ce
plt_poly_class_1d = let

	gr()
	f = f_quadratic2
	xys_ = hcat(f.(eachrow(xys))...)'
	plt = plot(ratio=1, framestyle=:zerolines, xlabel=L"x_1^2+x_2^2", ylabel="", labelfontsize=18, yaxis=false, ylim =[-0.2, 0.2], legendfontsize=15, size=(800,300), legend=:topright)
	...
	for x in xs
		...
		X_circle_new = hcat(f.(eachrow(X_circle))...)'
	for c in [0, 1]
		scatter!(X_circle_new[y_circle .== c, 1], X_circle_new[y_circle .== c, 2], c= c+1, ms=6, alpha=0.8, label="class $(c)")
	end
...
# ╔═╡ a5760db9-6104-41d3-b2a4-9a50f3566f78
plot([plt_poly_class_2d, plt_poly_class_1d]..., layout=(1,2), size=(800, 520) )

# ╔═╡ 55eb069e-06b7-45fa-8912-8fe3f2a71290
begin
...
# ╔═╡ 3563ed17-e560-4a24-acbb-d1aeecbed5fa
w_circle_2d = let
	X_circle_2 = [ones(size(X_circle)[1]) X_circle.^2]
	w0 = zeros(3)
	miters = 10
	for _ in 1:miters
		g = Zygote.gradient(w -> logistic_loss(w, X_circle_2, y_circle), w0)[1]
		H = Zygote.hessian(w -> logistic_loss(w, X_circle_2, y_circle), w0)[1]
...
# ╔═╡ aa817053-a52c-4caf-a3d2-415c430038eb
md"""

## Polynormial logistic regression
"""

# ╔═╡ 42d312c8-ad53-409e-97e4-ef39cb7d3e45
let
	plt1 = plot(xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", framestyle =:zerolines, labelfontsize=18, legendfontsize=10)

	w0 = w_circle_2d
	plot!(-1.5:0.1:1.5, -1.5:0.1:1.5, (x, y) -> logistic([1, x^2, y^2]' * w0), st=:surface, c=:jet, colorbar=false, alpha=0.5)

	for y in [false, true]
		scatter!(X_circle[y_circle .== y, 1], X_circle[y_circle .== y, 2], y * ones(sum(y_circle .== y)), c= y+1, label="class $(Int(y))", alpha=0.9, ms=3)
	end



	plt2 = plot(xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines, labelfontsize=18, legendfontsize=10, ratio=1)

	plot!(-1.5:0.1:1.5, -1.5:0.1:1.5, (x, y) -> logistic([1, x^2, y^2]' * w0), st=:contourf, c=:jet, levels= 0:0.1:1.0, colorbar=true, alpha=0.3, xlim=(-1.5, 1.5), ylim=(-1.5, 1.5))

		for y in [false, true]
		scatter!(X_circle[y_circle .== y, 1], X_circle[y_circle .== y, 2], c= y+1, label="class $(Int(y))", alpha=1.0, ms=3)
	end
	plot(plt1, plt2, size=(800,400))
end

# ╔═╡ fa139e32-0a2d-4129-93ff-39e235f86381
Xs_quad, y_quad=let
	Random.seed!(123)
	Xs_quad1 = rand(200, 2) * 2 .+ [0.1, 0.1]'
	
	Xs_quad = [Xs_quad1; Xs_quad1 * -1]

	Xs_quad = [Xs_quad; Xs_quad .* [-1, 1]']

	# Xs_quad = [Xs_quad; rand(200, 2) * 2 .- [0.1, 2.1]']
	ys_quad = (Xs_quad[:, 1] .* Xs_quad[:, 2]) .> 0
	# plt = plot()
	plt = plot(ratio=1, xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines, labelfontsize=18, legendfontsize=15)
	for c in [0, 1]
		scatter!(Xs_quad[ys_quad .== c, 1], Xs_quad[ys_quad .== c, 2], c= c+1, label="class $(c)")
	end
	Xs_quad, ys_quad
end;

# ╔═╡ 93457159-2e41-4833-9dd6-99dc4e54d345
md"""

## Polynomial logistic regression (example 2)

"""

# ╔═╡ a328d2bb-a2c0-4777-afd7-b045163c23f3
let
	plt = plot(ratio=1, xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines, labelfontsize=18, legendfontsize=15)
	for c in [0, 1]
		scatter!(Xs_quad[y_quad .== c, 1], Xs_quad[y_quad .== c, 2], c= c+1, ms=3, alpha=0.8, label="class $(c)")
	end
	plt
end

# ╔═╡ 10c3b8e0-6941-46a5-9b4a-8b86d7cd52c2
md"""

#### Introduce third dimension ``x_1\times x_2``
...
# ╔═╡ 0fee222f-c731-4db6-b9ba-81df33873388
md"""Move view angle: $(@bind ang1 Slider(-30:0.5:30; default =30)),  $(@bind ang2 Slider(-30:0.5:30, default =-15))"""

# ╔═╡ 9e47930e-651a-45d7-bed4-e32bfe8fb30b
let
	gr()

	plt = plot(xlabel=L"x_1", ylabel=L"x_2", zlabel=L"x_1\times x_2", camera=(ang1,ang2), framestyle=:zerolines)


	for c in [0, 1]
		scatter!(Xs_quad[y_quad .== c, 1], Xs_quad[y_quad .== c, 2], Xs_quad[y_quad .== c, 1] .* Xs_quad[y_quad .== c, 2],c= c+1, label="class $(c)", ms=3, alpha=0.6)
	end

	plt
end

# ╔═╡ 3b4d9519-3295-4631-9187-e7f422b66b18
let
	Xs_quad_ = [Xs_quad Xs_quad[:, 1].* Xs_quad[:, 2]]
	w0 = zeros(size(Xs_quad_)[2])
	miters = 5
	for _ in 1:miters
		g = Zygote.gradient(w -> logistic_loss(w, Xs_quad_, y_quad), w0)[1]
		H = Zygote.hessian(w -> logistic_loss(w, Xs_quad_, y_quad), w0)[1]
		w0 = w0 - (H + (1e-4)*I) \ g
	end
	plt1 = plot(xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", framestyle =:zerolines, labelfontsize=18, legendfontsize=10, camera=(ang1,ang2))
	for y in [false, true]
		scatter!(Xs_quad[y_quad .== y, 1], Xs_quad[y_quad .== y, 2], y * ones(sum(y_quad .== y)), c= y+1, label="class $(Int(y))", alpha=0.2, ms=2)
	end
	# w0 = w_circle_2d
	plot!(-4:.5:4, -4:.5:4, (x, y) -> logistic([x, y, x*y]' * w0), st=:surface, c=:jet, colorbar=false, alpha=0.5)
	...
	plot!(-2.5:0.1:2.5, -2.5:0.1:2.5, (x, y) -> logistic([x, y, x*y]' * w0), st=:contourf, c=:jet, levels= 0:0.1:1.0, colorbar=true, alpha=0.3, xlim=(-1.5, 1.5), ylim=(-1.5, 1.5))

		for y in [false, true]
		scatter!(Xs_quad[y_quad .== y, 1], Xs_quad[y_quad .== y, 2], c= y+1, label="class $(Int(y))", alpha=1.0, ms=3)
	end
	plot(plt1, plt2, size=(800,400))
end

# ╔═╡ 22c18a8b-44de-450d-ab2a-7cb993578c71
md"""

# Fixed basis expansion
"""

# ╔═╡ 354269f0-d0ee-4d13-9111-908ce1d4340a
md"""


## Question 

#### -- Can you spot a _key_ feature of some functions below?

\

```math
\Large
h_1(x) = w_0 + w_1 x + w_2 x^2 
```


```math
\Large
h_2(x) = w_0 + w_1 e^{-\frac{1}{2}(x-1)^2} + w_2 e^{-\frac{1}{2}(x-2)^2}
```


```math
\Large
h_3(x) = w_0 + w_1 \sin{x^2} + w_2 e^{x} 
```


```math
\Large
h_4(x) = w_0 +w_1 x + \frac{1}{1 + e^{-w_2 x}}
```
...
```math
\Large
h_6(x) = \sin(w_0 +  w_1 x) \cdot \exp\left \{-{(w_0 + w_1 x)^2}\right \}
```

"""

# ╔═╡ b1f269f8-c72d-441e-9f37-db269f200b13
md"""

## Linear models
"""

# ╔═╡ 6e6cac10-7107-4784-92e1-3a938c48aa28
md"""
```math
\Large
\boxed{\begin{align}
...
# ╔═╡ 3abba493-f71c-45f3-a663-ccf4808caae8
md"""

#### _They are linear_ with parameter ``\mathbf{w}``: *i.e.* $h(x; \mathbf{w}) = \mathbf{w}^\top \tilde{\mathbf{x}}$

$\Large h_1(x) = \begin{bmatrix}w_0 & w_1 & w_2\end{bmatrix}\begin{bmatrix}1 \\ x \\x^2 \end{bmatrix}$

$\Large h_2(x)  = \begin{bmatrix}w_0 & w_1 & w_2\end{bmatrix}\begin{bmatrix}1 \\ e^{-\frac{1}{2}(x-1)^2} \\e^{-\frac{1}{2}(x-2)^2} \end{bmatrix}$

$\Large h_3(x) = \begin{bmatrix}w_0 & w_1 & w_2\end{bmatrix}\begin{bmatrix}1 \\ \sin(x^2) \\ e^x \end{bmatrix}$

"""

# ╔═╡ 6c13a27e-519f-4a75-8ec5-29cbf2efd491
md"""
...
* ### they are non-linear: we cannot form ``h(x) \neq \mathbf{w}^\top\mathbf{\tilde{\mathbf{x}}}``

"""

# ╔═╡ e97d3b38-996e-4409-aa38-5c52d788fc18
md"""
...
!!! note "Linear models"
	$\Huge h(x) = \mathbf{w}^\top{\mathbf{x}}$
	* #### true that ``h(x)`` is a non-linear function for ``x``
	* #### but ``h(x)`` is a linear from ``\mathbf{w}``'s perspective 

"""

# ╔═╡ e94230fd-13ba-4492-8039-fa7d0d4f8e24
md"""
...
```math
\Large
h(x) = w_0 + \sqrt{e^{w_1}} x + \frac{x}{w_2} + \frac{1}{1 + e^{-w_3 x}}
```


* ##### which term is the problematic bit ?
...
# ╔═╡ e7bd8707-0179-4d2f-b6a0-ca027a50db5b
let
	gr()
...
#### More *generally*, *fixed basis expansion* via  ``K`` fixed *basis functions*

```math
\Large
\boldsymbol{\phi}=\;\;\{\textcolor{lightblue}{\phi_0(\mathbf{x})},
\;\;\;\;\;\textcolor{orange}{\phi_1(\mathbf{x})},\;\;\;\;\;\; \textcolor{green}{\phi_2(\mathbf{x})},\;\;\;\; \ldots,\;\;\;\textcolor{magenta}{\phi_K(\mathbf{x})}\}
```

* ##### each ``\phi_k(\mathbf{x})`` is a _scalar_ output function

##


#### The *expanded* design matrix is a ``n \times (K+1)`` matrix: 


```math
\Large
\mathbf{X} \xrightarrow[]{\{\phi_0,\, \phi_1,\, \ldots, \,\phi_K\}} \boldsymbol{\Phi}
```
...
```math
\Large
\boldsymbol{\Phi} = \begin{bmatrix}\columncolor{lightblue}\phi_0(\mathbf{x}^{(1)}) & \columncolor{orange}\phi_1(\mathbf{x}^{(1)}) & \columncolor{lightgreen}\phi_2(\mathbf{x}^{(1)})  & \ldots & \columncolor{magenta}\phi_K(\mathbf{x}^{(1)}) \\
\phi_0(\mathbf{x}^{(2)}) & \phi_1(\mathbf{x}^{(2)}) & \phi_2(\mathbf{x}^{(2)}) & \ldots & \phi_K(\mathbf{x}^{(2)})\\
\vdots & \vdots & \vdots & \ddots & \vdots \\
\phi_0(\mathbf{x}^{(n)}) & \phi_1(\mathbf{x}^{(n)}) & \phi_2(\mathbf{x}^{(n)}) & \ldots & \phi_K(\mathbf{x}^{(n)})
\end{bmatrix}
```
"""
...
$\boxed{\Huge \phi_k(x) = x^k}$
"""

# ╔═╡ 13fcbc7b-e145-4295-b8dd-925bd5eb5951
md"""

## Fixed basis expansion -- prediction


#### The *expanded* design matrix is ``n \times K+1`` 
...
#### The *predictions* are still linear 
...
#### Then, they reduce to linear/logistic regression's problem
...
# ╔═╡ 4fa96e3c-9a3e-4bc9-8b2a-f187cceae414
md"""Add extra data: $(@bind add_extra_x CheckBox(default=false)); Make ``\phi`` location aware: $(@bind make_mu CheckBox(default=false)); Move location ``\mu``: $(@bind location_par Slider(-3:0.1:6; default = 3.1))"""

# ╔═╡ 78daf3d9-8070-43ea-a9a1-970e317c3006
let
		plt = plot(title="Polynomial basis functions", legend=:outerright, xlabel=L"x", labelfontsize=20, legendfontsize=12, size=(850,500), framestyle=:zerolines)
		for k in 1:10
			if !add_extra_x
				plot!(-1:0.02:1, (x) -> x^k, label=L"\phi_{%$(k)}(x) = x^{%$(k)}", lw = (10-k)/2)
			else
				if make_mu
					plot!(range(location_par -3, location_par +3, 50), (x) -> (x-(location_par))^k, label=L"\phi_{%$(k)}(x;\mu) = (x-\mu)^{%$(k)}",  ylim =[-15,15], lw=2)
				else
					if k < 9
					plot!(-5:0.1:5, (x) -> x^k, label=L"\phi_{%$(k)}(x) = x^{%$(k)}", lw = 2, ylim =[-10, 10])
					end
				end
			end
		end
		...
		if true
			Random.seed!(4321)
			xs = rand(12) * 2 .- 1
			scatter!([xs], [zeros(length(xs))], m=:x, c=:gray, ms=6, markerstrokewidth=3, label="training "*L"x")

			if add_extra_x
				Random.seed!(456)
				xs = rand(8) * 1.2 .+ 2.5
				# push!(xs, rand(12) * 1.2 .- 2.0)
				scatter!([xs], [zeros(length(xs))], m=:x, c=:red, ms=6, markerstrokewidth=3, label="extra training "*L"x")
				# for k in 1:5
				# plot!(range(location_par -3, location_par +3, 50), (x) -> (x-(location_par))^k, label="",  ylim =[-15,15], lw=2)
				# end
...
## Location based basis -- radial basis functions
...
```math
\Large
\phi(x; \mu, s)=\text{rbf}(x;\mu, s) = \exp\left\{ -\frac{1}{2s} (x-\mu)^2\right\}
```
...
let
	plt_rbf_basis = plot(title="RBF basis functions", framestyle=:origin, legendfontsize=12, size=(800,400))
	s = s_
...
```math
\Large
\phi(x; \mu, s) = \frac{1}{1+ e^{-\frac{(x-\mu)}{s}}}
```
...
# ╔═╡ 7c82512f-e249-42b0-ac45-6da3f52b72df
md"""

## Location based basis -- `tanh`
...
```math
\Large
\phi(x; \mu) = \tanh(x; \mu) = \frac{e^{2(x-\mu)} - 1}{e^{2(x-\mu)} + 1}
```

* ##### a squash function: squeeze to ``(-1, 1)``

"""

# ╔═╡ bbafed0f-60d6-4342-ae03-71c9f22c3668
let

	gr()
	plt_tanh_basis = plot(title="tanh basis functions", framestyle=:origin)
	s_sig = sl_
	for μ in -1:0.1:1
		plot!(-1.5:0.01:1.5, (x) -> LogExpFunctions.tanh((x-μ)/s_sig),ylim=[-1.3, 1.3], lw =1,lc=:gray, label="")
	end

	plot!(-1.5:0.01:1.5, (x) -> LogExpFunctions.tanh((x -μl_)/s_sig), lw =3,lc=:red, label=L"\tanh(x; \mu = %$(μl_))", legend=:outerbottom)
...
* ##### we need to specify the expansion locations: 
...
* ##### select  with evenly spaced ``K`` locations within the data range


* ##### randomly select ``K`` points within ``\mathbf{x}``'s range 


* ##### randomly choose ``K`` from training data ``\boldsymbol{\mu}_k \in \{\mathbf{x}^{(1)}, \ldots, \mathbf{x}^{(n)}\}``
...
md"""Add Basis functions: $(@bind add_basis_2 CheckBox(default=false)), Add range: $(@bind add_data_range CheckBox(false)), 
...
## Demonstration -- two datasets


"""

# ╔═╡ fc954781-6a79-46ae-a50b-4a70c20bdd59
md"Observation noise scale: $(@bind noise_scale Slider([0, exp.(-5:0.1:-1)...], default=0.05))";

# ╔═╡ 88b56ef8-40aa-41fe-9a16-c892ac7505f2
begin
	Random.seed!(123)

	function f_half_circle(x; r= 1)
		if abs(x) <= r
			sqrt(r^2 - x^2)
		else
			0
	...
	nobs_hc = 15 
	xs_hc = collect(range(-2, 2, nobs_hc))
	ys_hc = f_half_circle.(xs_hc) .+ randn(nobs_hc) * noise_scale
end;

# ╔═╡ 88c1bb3e-dab8-459a-b2d0-ebc2496f5b19
md"Add true function: $(@bind addsin CheckBox(default=false)), add another: $(@bind add_hc CheckBox(default=false))"

# ╔═╡ 3bfb67fe-0d00-4bcd-80f0-aee3b9bd6bc4
let
	plt = plot()
	if addsin
		plt = plot(label="training data", title="Regression training data")
		plot!(xs_q4, ys_q4, st=:scatter, label="")
		plot!(0:0.1:2π, true_f, label="true signal", lc=1, lw=2, title=L"\sin(x)", xlabel=L"x")
		plt
	end
	if add_hc 
		plt = plot(label="training data", title="Regression training data")
		plot!(xs_hc, ys_hc, st=:scatter, label="training data")
		plot!(-2:0.01:2, f_half_circle, ratio=1, lw=2,lc=1, framestyle=:zerolines, label="")
		plt
	end
	plt
end
...
md"Number of basis: $(@bind n_basis Slider(1:1:20; default=1, show_value=true))"
...
begin
	...
	tanh(s=1) = (z) -> LogExpFunctions.tanh(z/s)
end;

# ╔═╡ e9bfad75-eb43-4212-a141-0e0655c48758
md"Basis function: $(@bind ϕ Select([rbf, relu, sigmoid, tanh])), 
Scale parameter ``s`` (if applied): $(@bind scale_ Slider(exp.(-5:0.1:1);default=0.02, show_value=true))
"
...
begin
	...
	μs = range((extrema(xs_q4) .+ (-1.5, 1.5))..., n_fixed_basis+2)[2:end-1]
end;

# ╔═╡ 61b18994-e47d-47f4-b806-83ddf1b3c5da
begin
	Random.seed!(111)
	n_fixed_basis_hc = n_basis
	μs_hc = range((extrema(xs_hc) .+ (-1.5, 1.5))..., n_fixed_basis_hc+2)[2:end-1]
end;

# ╔═╡ ce4f4d96-3f01-46d2-ba1e-6108a7af9141
linear_reg(Φ, y; λ = 1e-10) =  (Φ'Φ + λ*I) \ Φ'*y;

# ╔═╡ 852d5317-3f62-4ac8-9b8a-ab0b7869b269
let
	basis_fun = ϕ(scale_)
	plt = plot(xs_q4, ys_q4, st=:scatter, label="training data", ylim=[-1.25, 1.25], framestyle=:origin, size=(800,500), legend=:topright, legendfontsize=12)
	for (j, μ) in enumerate(μs)
		if j == 1
			plot!(-1:0.05:2π+1., (x) -> basis_fun(x-μ), lw=1.5, ls=:dash, lc=:gray, label="fixed basis functions")
		else
			plot!(-1:0.05:2π+1., (x) -> basis_fun(x-μ), lw=1.5, ls=:dash, lc=:gray, label="")
		end
	end
	
	plot!(-1.5:0.1:(2π+1.5), true_f, label="true signal", lc =:blue, lw=1.5, title="Fixed basis expansion regression")

	if add_fixed_fit
		Φ = basis_expansion(xs_q4, μs, basis_fun; intercept=false)
		w = linear_reg(Φ, ys_q4; λ = 1e-5)
		plot!(-1.:0.01:2π + 1., (x) -> (basis_expansion([x], μs, basis_fun;  intercept=false) * w)[1] , label="fixed basis regression", lw=3, lc=2, ylim=[-1.25, 1.25])
	end
...
# ╔═╡ 2b07d157-bf11-4c67-8732-d8a080f59a07
let
	true_f = f_half_circle
	basis_fun = ϕ(scale_)
	plt = plot(xs_hc, ys_hc, st=:scatter, label="training data",xlim =[-3, 3], ylim=[-1., 1.25], framestyle=:origin, ratio=1,  legendfontsize=12, legend=:bottomright, size=(800,500))
	for (j, μ) in enumerate(μs_hc)
		if j == 1
			plot!(range(μ -5, μ+5, 200), (x) -> basis_fun(x-μ), lw=1.5, ls=:dash, lc=:gray, label="fixed basis functions")
		else
			plot!(range(μ -5, μ+5, 200), (x) -> basis_fun(x-μ), lw=1.5, ls=:dash, lc=:gray, label="")
		end
	end
	
	plot!(range(extrema(xs_hc) .+ [-3, 3]..., 500), true_f, label="true signal", lc =:blue, lw=2, title="Fixed basis expansion regression")

	if add_fixed_fit
		Φ = basis_expansion(xs_hc, μs_hc, basis_fun; intercept=false)
		w = linear_reg(Φ, ys_hc; λ = 1e-5)
		plot!(range(extrema(xs_hc) .+ [-5, 5]..., 500), (x) -> (basis_expansion([x], μs_hc, basis_fun;  intercept=false) * w)[1] , label="fixed basis regression", lw=3, lc=2)
	end
...
let
	Random.seed!(4321)
	y_poly_new = (y_poly .- mean(y_poly)) / std(y_poly) + randn(length(y_poly))/6
	xmin, xmax = extrema(x_poly)
	plt = plot(x_poly, y_poly_new, st=:scatter, ylim=[-2.2,2.2], label="training data", title="", ms=6, alpha=0.6, framestyle = :origin, xlim = [xmin-0.5, xmax+0.5], legend=:outerbottom)
	K = 6
	xmin, xmax = extrema(x_poly)
	...
	if add_basis_2
		for (j, μ) in enumerate(μs)
			if j == 1
				plot!(xmin-0.5:0.01:xmax+0.5, (x) -> basis_fun(x-μ), lw=2, ls=:dash, lc=:red, label="basis functions")
			else
				plot!(xmin-0.5:0.01:xmax+0.5, (x) -> basis_fun(x-μ), lw=2, ls=:dash, lc=:red, label="")
			end
		end
		plot!(μs, zeros(K), st=:scatter, markershape=:x, mc =:red, ms=7, markerstrokewidth=4, label="")
	
		if basis_locations == "random from x"
			plot!(x_poly[μs_idx], y_poly_new[μs_idx], st=:sticks, mc =:red, alpha=1, ms=6, lc=:red,lw=2, ls=:dash, label="")

			scatter!(x_poly[μs_idx], y_poly_new[μs_idx], mc =:red, ms=6, markerstrokewidth=6, m=:x, label="")
			# plot!(x_poly[μs_idx], y_poly_new[μs_idx], st=:scatter, ylim=[-2,2], label="training data", title="", ms=6, alpha=0.6, framestyle = :origin, xlim = [xmin-0.5, xmax+0.5], legend=:outerbottom)
		end
	end
	plt
end

# ╔═╡ b4a45ba8-9eea-4c72-9778-c2b88c058157
begin
	Random.seed!(123)
	# nobs_circles = 500
	X_circle2, y_circle2 = MLJBase.make_circles(200; noise=0.15)
	# Matrix(X_moon)
	X_circle2 = matrix(X_circle2)
	y_circle2 = Array(y_circle2)
end;

# ╔═╡ 295c51b3-f2c2-423d-b6af-868e032289f5
md"""

## Logistic regression : with `rbf`
"""

# ╔═╡ f76fa216-1c17-4919-b7e4-4fc7ab3bac1a
scale_rbf_logistic = 0.05

# ╔═╡ 72bdde1a-fa06-4d7e-baaa-8ac6d703efad
md"Add rbf basis locations: $(@bind add_rbfs_ CheckBox(false)), Add rbf fit: $(@bind add_rbfs_fit CheckBox(false))"

# ╔═╡ b124b12a-0d96-4e54-8a03-44005f12fcc6
md"Number of basis: $(@bind k_basis_rbf Slider([20, 50, 80, 100, 120, 180, 200], default= 20))"

# ╔═╡ f5894d4e-50c5-4b24-a837-edfd4a093bed
w_fit, μs_rbf=let
	Random.seed!(123)
	KK = k_basis_rbf
	n_total = size(X_circle2)[1]
	basis_idx = shuffle(1:n_total)[1:KK] 
	# μs = rand(KK, 2) .* 2 .- 1.

	μs = X_circle2[basis_idx, :]
	Φ = ones(n_total, KK+1)
	rbf(X, μ, s = scale_rbf_logistic) = exp.(- 0.5 * sum((X .- μ).^2, dims=1) ./ s)[:]
	for k in 1:KK
		Φ[:, k+1] = rbf(X_circle2', μs[k,:])
	end
	...
	# for k in 1:KK
		w0 = zeros(size(Φ)[2])
		for _ in 1:miters
			g = Zygote.gradient(w -> logistic_loss(w, Φ, y_circle2) + λ * w'*w, w0)[1]
			H = Zygote.hessian(w -> logistic_loss(w, Φ, y_circle2) + λ * w'*w, w0)[1]
			w0 = w0 - H \ g
		end
...
# ╔═╡ 03e2e1db-8946-4a31-8fe1-870dba7089a8
let
	w0 = w_fit
	rbf(X, μ, s = scale_rbf_logistic) = exp.(- 0.5 * sum((X .- μ).^2, dims=1) ./ s)[:]
	function expand_(x, μs, s = scale_rbf_logistic) 
		rbf(x, μs, s)
	end
	plt2 = plot(xlabel=L"x_1", ylabel=L"x_2", framestyle =:zerolines, labelfontsize=18, legendfontsize=10, ratio=1)

	if add_rbfs_
		scatter!(plt2, μs_rbf[:, 1], μs_rbf[:, 2], m=:x, ms=5, markerstrokewidth=2, c=:gray, label="basis "*L"\mu", title="Number of basis is $(length(w0)-1)")

		ts = 0 : 0.1: 2π
		r = sqrt(scale_rbf_logistic) 
		xs = cos.(ts) * r
		ys = sin.(ts) * r
		for μ in eachrow(μs_rbf)
			plot!(plt2, μ[1] .+ xs, μ[2] .+ ys, st=:path, label="", lc=:gray)
		end
	end

	if add_rbfs_fit
		plot!(-1.5:0.01:1.5, -1.5:0.01:1.5, (x, y) -> (expand_([x, y], μs_rbf')' * w0[2:end] + w0[1]) > 0.0 , st=:contour, c=:jet, colorbar=false, alpha=1.0, xlim=(-1.5, 1.5), ylim=(-1.5, 1.5), legend=:outerbottom, size=(600,600), title="Number of basis is $(length(w0)-1)")
	end

	
	for y in [false, true]
		scatter!(X_circle2[y_circle2 .== y, 1], X_circle2[y_circle2 .== y, 2], c= y+1, label="class $(Int(y))", alpha=1.0, ms=3)
	end
...
manifest_format = "2.0"
project_hash = "1e838b0629d85ab632a187769b3c9606b02e581e"
