...
# ╔═╡ d597ceb6-a181-4038-834e-334625e5397e
md"""
...
# ╔═╡ 84691fad-48f6-4923-ab8f-7e565a3bac84
# md"""
...
# ╔═╡ 1cf0f0b2-cc6e-40b0-b6ef-8a3fe11bb2ce
# md"""

# #### for ``d`` dimensional ``\mathbf{x} \in \mathbb{R}^d``
...
# ╔═╡ 5d6c6018-4cd7-4b24-934e-d013c4064581
md"""
#### Demonstration
"""

# ╔═╡ 7d06e5b8-356e-4a3a-849a-33a2e1a04c27
let
	Random.seed!(111)
	dim = 4
	nobs = 1000
	## randomly generate μ and Σ
	trueμ = randn(dim)
	L = LowerTriangular(randn(dim, dim))
	# a simple way to generate random symmetric and positive definite matrix 
	# also known as cholesky decomposition
	trueΣ = L * L'
	## randomly generate 500 Gaussian samples x ∼ N(trueμ, trueΣ)
	xs = rand(MvNormal(trueμ, trueΣ), nobs)'

	## MLE for μ
	μ_mle = mean(xs, dims=1)[:]
	## MLE for Σ
	error = (xs .- μ_mle')
	Σ_mle = (error' * error)/nobs

	trueμ, μ_mle, trueΣ, Σ_mle
end
...
# ╔═╡ 5aa98443-8d0d-4804-be6f-16d8ea3f7823
md"
## `Discriminative` _vs_ `Generative`
"

# ╔═╡ ef9b306b-d50b-4310-adb1-8ce1cb962ec9
TwoColumn(
md"""

#### Prob. `discriminative` models



$(show_img("pdis_model.svg", w=1000))


* ##### only ``y`` are random

* _e.g._ logistic/softmax regression, most neural net
...
# ╔═╡ ae8bfb4d-4468-4c18-bc37-04a0ed07d7da
md"
## `Discriminative` _vs_ `Generative`
"

# ╔═╡ 22b5a5a7-cfa2-4f0f-b2bd-d499578ce244
TwoColumn(
md"""
...
$(show_img("pgen_model.svg", w=1000))



* ##### both ``\mathbf{x}`` and ``y`` are random with _joint_ distribution
* _e.g._ LDA, QDA, Naive Bayes and many others
	"""
...
#### And the model is called *Gaussian discriminant analysis* (GDA)
...
$(show_img("gaussiandis.svg", w=440))



""", html"""<br/><br/><br/><center><img src="https://leo.host.cs.st-andrews.ac.uk/figs/CS5914/gda_bnalone.svg" width = "180"/></center>
""")
...
* #####  ``y^{(i)} \in \{\text{Adelie}, \text{Chinstrap}, \text{Gentoo}\}`` categorical
  * ##### ``p(y)``: prior distribution over three species

* #####  ``\mathbf{x}^{(i)}``: bill length/depth of an animal given species ``y^{(i)}``
  * ##### ``p(\mathbf{x}|y=\text{Chinstrap})``: a Gaussian (red cluster)
"""
...
#### -- the full model ``p(y^{(i)}, \mathbf{x}^{(i)}|\mathbf{\Phi})`` with the parameters $\mathbf{\Phi} = \left \{\boldsymbol{\pi},  \{\boldsymbol{\mu}_c, \boldsymbol{\Sigma}_c \}_{c=1}^C\right \}$
"""
...
# ╔═╡ 617fb674-da62-4595-bbf7-3a440b5212d8
md"""
...
# ╔═╡ 8336b9fa-96bb-4d39-abac-10649508cade
...
# ╔═╡ 2e74fa77-87f9-4303-8cdc-66b99bab1121
show_img("gdabayes.svg", w=700)

# ╔═╡ 4091f85c-e4ad-482f-af60-220df929c6eb
...
* ##### The denominator: a _normalising constant_ for the posterior
...
md"Select ``x``: $(@bind x00 Slider(-4.5:0.1:4.5, default=-2, show_value=true))"
...
# ╔═╡ c6bbb609-ac73-4f39-8c3b-6f0edc39aa50
md"""

## Classification shortcut (MAP estimator)

#### To find the *most likely* $\hat{y}$, 
...
> \begin{align}
> \hat{y}_{\text{MAP}} 
> &= \arg\max_c\;  p(y^{(i)}=c|\mathbf{x}^{(i)})
> \end{align}
> ```


* ##### known as maximum aposteriori estimator (MAP), we do not need to normalise
...
md"Select ``x``: $(@bind x0 Slider(-5.5:0.1:5.5, default=-2, show_value=true))"
...
#### Furthermore, apply ``\ln``-transform (in-variant to argmax)
...
* ##### "log" further simplifies computation
...
* ##### where ``\mathbf{w}_c = \mathbf{\Sigma}^{-1}\boldsymbol{\mu}_c`` and ``b_{c} = \ln(\pi_c)  -\frac{1}{2} \boldsymbol{\mu}_c^\top\mathbf{\Sigma}^{-1}\boldsymbol{\mu}_c``
...
* ##### which simply corresponds to _diagonal_ covariance
"""
...
```math
\Large
\ell(\boldsymbol\Theta) = \ln p(\mathcal{D}|\mathbf{\Theta})
```
""", 
...
$$\Large\hat{\boldsymbol{{\Phi}}} \leftarrow \arg\min_{\boldsymbol{\Theta}}\, {-\ln p(\mathcal{D}|\boldsymbol{\Theta})}$$
...
# ╔═╡ f9c19f00-e3f1-43ff-a6b5-442f20de46ab
md"""

# MAP _vs_ MLE 
#### -- a second look at ridge regularisation
"""

# ╔═╡ 70078839-0f8b-4033-9a1e-0a8921c78645
md"""
## Still remember ? ...
\
...
# ╔═╡ fe961447-7b97-41a1-9e11-8b9916ad82de
md"""
...
# ╔═╡ 26d9f46b-92e9-49cd-97ba-3d92531e8bfb
TwoColumn(md"""
\
\
...
$$\large\hat{h}_{\text{MLE}} \leftarrow \arg\max_h P(\mathcal{D}|h)$$



* ##### ``h_{\text{MLE}} = \text{cancer}``
...
# ╔═╡ 9fc63dfa-06fe-4ea8-b331-71b2a679e787


# ╔═╡ 09b7e7a9-112a-48db-ad54-928db64ff594
md"""
...
# ╔═╡ 03b1d781-2b02-44bf-83aa-f6ac378fcbae
TwoColumn(md"""
...
$$\Large h_{\text{MAP}} = \arg\max_h  P(h|\texttt{cough})$$

""", html"""<center><img src="https://leo.host.cs.st-andrews.ac.uk/figs/cough_bn.png" width = "100"/></center>
""")

# ╔═╡ c11b87c4-9d61-41e7-89d7-76e3bec4bf6f
md"""


#### The posterior, by Bayes' rule

$$\large\begin{align}P(h|\texttt{cough}) \propto P(h) P(\texttt{cough}|h) = \begin{cases} 0.89 \times 0.05 & h=\texttt{healthy} \\ 0.1 \times 0.45 & h=\texttt{cold} \\ 0.01 \times 0.54 & h=\texttt{cancer}  \end{cases}
\end{align}$$
...
# ╔═╡ 6a904fee-c1b8-48fb-a23f-7d6ad09d01be
# md"""
...
# ╔═╡ 18cb8efd-247d-4ae8-9091-b8a9054fd558
# TwoColumn(
# md"""
...
# $$\Large\hat{h}_{\text{MAP}} \leftarrow \arg\max_h P(h|\mathcal{D})$$
...
# ╔═╡ c0f3e979-81f2-4608-b81f-35a841562ab5
begin
...
# ╔═╡ 889ad3be-4ae7-415e-b8bf-34bac821051d
TwoColumn(md""" 
\
...
"""
	...
	, 
	begin
prior_plt=plot(prior_p, fill=true, st=:bar, c=1:3, xticks=(1:3, Hs), markershape=:circ, ylim =[0, 1.0], label="", title="Prior: "*L"P(h)", size=(300,300))
end)

# ╔═╡ ca838584-944d-4766-b693-0a2544584e68
TwoColumn(md"""
\


$$\large \begin{align} P(h&|\mathcal{D}) \\
&= \begin{cases}\color{blue} 0.470 & \colorbox{lightblue}{h = \texttt{hthy}} \\
\color{red}0.471 & \colorbox{orange}{h = \texttt{cold}} \\
\color{green}0.057 & \colorbox{lightgreen}{h = \texttt{cancer}}
\end{cases} 
\end{align}$$
...
$$\large \boxed{h_{\text{MAP}} = cold}$$
""", begin
	# l = @layout [a b; c]
	post_plt = Plots.plot(post_p, seriestype=:bar, c=1:3, markershape=:circle, label="", xticks=(1:3, Hs), ylim=[0,.5], ylabel="", title="Posterior: "*L"P(h|\texttt{cough})" ,legend=:outerleft, size=(320,300))
	# plt_cough = Plots.plot(prior_plt, like_plt, post_plt, layout=l)
end)

# ╔═╡ d6dc353d-832c-4782-aad9-3f94921f6107
md"""
## Recap: probabilistic linear regression


##### Unknown hypothesis: ``\mathbf{w}`` (assume ``\sigma^2`` is known)
...
##### And the _negative_ log likelihood is 

$$\large \begin{align} \mathcal{L}(\mathbf{w}) = -\ln p(\mathbf{y}|\mathbf{w}, \sigma^2, \{\mathbf{x}^{(i)}\}) 
&= \frac{1}{2\sigma^2} \underbrace{\sum_{i=1}^n({y}^{(i)}-\mathbf{w}^\top \mathbf{x}^{(i)})^2}_{\text{sum of squared error loss!}} + C
\end{align}$$

"""

# ╔═╡ 0c262f06-1774-4198-85e7-40ac43f22dab
md"""
...
```math
\Large
	\hat{\mathbf{w}}_{\text{MAP}}\leftarrow \arg\min_{\mathbf{w}} \underbrace{\mathcal{L}(\mathbf{w}) + \frac{\lambda }{2} \mathbf{w}^\top \mathbf{w}}_{\text{regularised loss}}
```


"""

# ╔═╡ 18f4872a-77e2-4882-bb38-8b8d0f91181f
md"""
## MAP estimator
"""

# ╔═╡ 555b512f-8924-468b-9bac-95499a92bd28
TwoColumn(md"""


##### Introduce a Gaussian prior ``p(\mathbf{w})`` 


$\Large \begin{align}p(\mathbf{w})&= \prod_{j=1}^m \mathcal{N}(w_j; 0, 1/\lambda)
\end{align}$

* independent Gaussians
  * zero mean: $0$
  * variance is: $1/\lambda$

""",

show_img("bayeslinreg2.svg", h=180))

# ╔═╡ 9ad720d4-665b-4016-a968-4ed12b6e580b
md"""
...
##### Assume a zero mean Gaussian prior for $\mathbf{w}$
...
# ╔═╡ 5b3cfbbd-b731-4d1e-97d7-0518449c45e1
md"""
## MAP ⇔ Regularisation
...
# ╔═╡ f108fa6e-359d-449f-ae5a-d3761ba148b8
md"""
## MAP ⇔ Regularisation
...
$$\large
\begin{align}\mathbf{w}_{\text{MAP}}&=\arg\max_{\mathbf{w}} 
\;\colorbox{lightblue}{$\ln  {p(\mathbf{w})}$}_{\small\text{log-prior}} +\colorbox{lightsalmon}{$\ln {p(\mathbf{y}|\mathbf{X}, \mathbf{w}, \sigma^2)}$}_{\small\text{log-lik}} \\
&= \colorbox{lightblue}{$-  \frac{\lambda}{2} \sum_j w_j^2 $} \, \colorbox{lightsalmon}{$-\frac{1}{2}\sum_{i=1}^n (y^{(i)}- \mathbf{w}^\top \mathbf{x}^{(i)})^2$} + C
\end{align}$$



##### which is the same as `Ridge` regression
$$\large
\begin{align}\mathbf{w}_{ridge} =\arg\min_{\mathbf{w}}
\colorbox{lightblue}{$ \frac{\lambda}{2} \sum_j w_j^2 $} \, +\colorbox{lightsalmon}{$\frac{1}{2}\sum_{i=1}^n (y^{(i)}- \mathbf{w}^\top \mathbf{x}^{(i)})^2$} + C
\end{align}$$
...
!!! note ""
	 ...
	 * ##### _explains all ML practices_

"""

# ╔═╡ c40520ff-7dec-428e-906e-aed493087b33
aside(tip(md"""
We assume $\sigma^2=1$ here for simplicity; but it is not necessary to show the equivalence.
"""))
...
* see below for a demonstration, where we have computed $\ln \boldsymbol{\pi}$ with `logsoftmax` directly (which is more numerical stable)


The negative average log likelihood for $\boldsymbol{\pi}$ is 

$$-\frac{1}{n}\mathcal L(\boldsymbol{\pi}) = -\frac{1}{n}\sum_{i=1}^n \sum_{k=1}^K {\mathbb{1}(y^{(i)}=k)} \cdot \ln \pi_k+ C$$
"""

# ╔═╡ 66440f3a-65b4-4f4e-8158-d886138044e3
let
	Random.seed!(123)
	## simulate 1000 observations from a categorical distribution with true parameter [0.1, 0.2, 0.7]
	trueπ = [.1, .2, .7]
	ys = sample(1:3, Weights(trueπ), 1000)
	ϕ = randn(3) # free parameter for π 
	γ = 1.0 # learning rate
	for i in 1:100
		...
		ϕ .-= γ * gs[1] ## gradient descent 
	end
	π_mle_gradient_descent = softmax(ϕ)
	π_mle_closed_form = counts(ys) ./ length(ys)
	## transform the parameter back; MLE of π
	trueπ, π_mle_gradient_descent,  π_mle_closed_form
end

# ╔═╡ 976711c5-009d-4feb-bc14-d262e250fdc5
md"""
### Reparameterize symmetric positive definite matrix $\mathbf{\Sigma}$

The covariance matrix  $\mathbf{\Sigma}$ is a bit more involved, but still doable. Note that the covariance matrix needs to satisfy:
* symmetric
* positive definite 

* and also the square root of the diagonal entries, *i.e.* the marginal standard deviations, have to be strictly positive, *i.e.* $\sqrt{\mathbf{\Sigma}_{ii}} = \sigma_i > 0$

To directly optimise $\mathbf{\Sigma}$ via Gradient descent is not possible (gradient descent will very likely to violate the constraints), instead we reparameterise $\mathbf{\Sigma}$ by cholesky decomposition:

$\mathbf{\Sigma} = \mathbf L\mathbf  L^\top,$ 

* where $\mathbf L$ is a lower triangular matrix (and each entry on the lower triangular part is unconstrained: *``L_{i\geq j}\in (-\infty, +\infty)``*):
...
* to make the computation more stable and also for the reason that ${L}_{ii} =\sigma_i >0$, we further constrain the diagonal entries of $\mathbf L$ to be strictly positive (this is not strictly required but definitely recommended for numerical stability reasons), and reparameterise them by using the standard $\exp$ transformation:
...
* in summary, after the reparameterisation, the parameters for $\mathbf{\Sigma}$ become $\mathbf L^\ast,$ where $L_{dd} = \exp(L^\ast_{dd}),$ and $L_{dd'} = L_{dd'}^\ast$ for $d\neq d'$.

* you can think the above transformation as a multivariate generalisation of the reparameterisation of the scalar variance $\sigma^2$:  $\sigma^2 = \exp(\ln \sigma) \cdot \exp(\ln\sigma)).$ And we optimise $l^\ast = \ln \sigma$ instead. 

##### Note that the parameters $\{L_{i \geq j}^\ast\}$ are uncontrained at all, we can safely apply gradient descent.

* most importantly, $\mathbf{\Sigma} = \mathbf L^\ast\mathbf  {L^\ast}^\top$ satisfies all the constraints desired for the covariance matrix (it is left as an exercise to show that the claim is true, *i.e.* the covariance matrix is symmetric and positive definite via the reparemeterisation)
* To make our life easier when doing gradient descent, it is a good idea to wrap this reparameterisation as a packing/unpacking method.
...
begin
	# transform L* to Σ
	function unpackΣ(L)
		# exp the diagonal entries
		LL = expdiag(L)
		# return the covariance matrix as cholesky decomposition
		Σ = LL * LL'
		return Σ
	end
	...
	function expdiag(array)
		## Zygote does not support in place mutation, so we have to use Buffer array instead
...
# ╔═╡ e5ce6a10-51c1-4f79-b23d-1a65034591ac
md"""

#### Specify the log-likelihood of the QDA model

Good idea to do minimisation rather than maximisation, so we compute negative per sample log likelihood

$$-\frac{1}{n}\mathcal L(\Phi) =- \frac{1}{n}\left \{\sum_{i=1}^n \sum_{k=1}^K {\mathbb{1}(y^{(i)}=k)} \cdot \ln \pi_k +\sum_{k=1}^K  \sum_{i=1}^n {\mathbb{1}(y^{(i)}=k)} \cdot \ln \mathcal{N}(x^{(i)}|\mu_k,\Sigma_k)\right \}$$

* *i.e.* mean negative log likelihood
"""
...
function gda_negative_loglik(data, ys, ϕ, μs, Ls; LDA=false)
	n = length(ys)
	logπ = logsoftmax(ϕ)
	## nll for ys; i.e. cross entropy loss
	nll = - sum(logπ[ys])
	K = length(ϕ)
	## if LDA, then Σ is shared; and Ls is just one lower triangler matrix 
	if LDA
		Σ = unpackΣ(Ls)
	end
	# for each class y == k
	for k in 1:K
		...
		mvn = MvNormal(μ, Σ)
		nk = sum(ys .== k)
		if nk > 0  ## if no observation in k th class, we can skip it 
			nll -= loglikelihood(mvn, data[ys .== k, :]')
		end
	end
	return nll/n
end

# ╔═╡ dc58f4e9-6de0-426d-b58a-cc6fc04cabbc
md"""

### Gradient descent (auto-diff with `Zygote.jl`)


You can derive the gradient yourself but not necessary. We can just use auto-diff such as `Zygote.jl`.

`DistributionsAD.jl` is a very handy package that specifies how we can auto-diff some common distributions efficiently (it tells auto-diff package such as `Zygote.jl` how to propagate the gradient through the log density such as ``\nabla_{\boldsymbol{\mu}}\ln \mathcal{N}(\mathbf{x}; \boldsymbol{\mu}, \mathbf{\Sigma}), \nabla_{\boldsymbol{\Sigma}}\ln \mathcal{N}(\mathbf{x}; \boldsymbol{\mu}, \mathbf{\Sigma})``); it is optional to add it but it should speed up your code a lot.


"""

# ╔═╡ 02e6d875-af22-466b-a81d-1fa62b5369e2
md"""

#### Demonstration

We use some simulated QDA data here with known parameters; here we have $C=3$ classes.

* there are 800 observations $\{y^{(i)}, \mathbf{x}^{(i)}\}_{i=1}^{800}$ simulated based on the QDA model

* the objective is to learn the true parameters by MLE via gradient descent
"""

# ╔═╡ 71029d31-4689-44af-aa8c-d0a154edd8c4
truemvns₂, trueπs₂ ## true parameters 

# ╔═╡ c7bd2778-5e3a-4a50-974c-dc2a9a23f1c0
data₂ ## the training data

# ╔═╡ 02002d81-0b36-4b55-87b7-5778f6942014
md"""

#### Gradient descent (QDA)
"""

# ╔═╡ ca675aba-8031-47d7-a2b2-2aa8d37cda00
losses_qda_gd, πs_qda_gd, μs_qda_gd, Σs_qda_gd = let
	Random.seed!(123345)
	## randomly initialise the free parameters for gradient descent
	ϕ = randn(3) # π
	μs = randn(2, 3) # μs
	Ls = zeros(2, 2, 3) # Σs 
	...
	# g[3]
	losses, softmax(ϕ), μs, [unpackΣ(Ls[:,:,k]) for k in 1:3 ]
end;

# ╔═╡ 04907f8a-a95a-4419-a44f-a5b8c158cd9d
plot(losses_qda_gd, title="QDA learning with Gradient descent", label="", xlabel="Iterations", ylabel="Negative log likelihood")

# ╔═╡ cedf17d4-ca46-4dd4-8339-29bcabce0682
md"""

We compare the gradient descent with the closed form solution. If you run the gradient descent long enough, they should return the same results (subject to numerical floating error.) You may also want to use Newton's method to speed up the optimisation. 

No surprise here: at the end of the day, they are solving the same problem (maximum likelihood estimation) with two different approaches, one iterative gradient descent and one is closed form solution.
"""

# ╔═╡ 6f5a7295-1b10-4a99-89fc-669e61187e5f
πs_qda_gd, μs_qda_gd, Σs_qda_gd ## the learnt parameters via Gradient descent

# ╔═╡ 2f8e92fc-3f3f-417f-9171-c2c755d5e0f0
π_qda_closed, μ_qda_closed, Σ_qda_closed= let
	### QDA learning with closed form solution
...
# ╔═╡ dacae836-661f-421f-8a9b-c17976bee3a4
π_qda_closed, μ_qda_closed, Σ_qda_closed ## the learnt parameters via closed form solution; i.e. pooled mean and covariances

# ╔═╡ 088d1b71-5c3b-47c0-9e2c-7249125c00d1
md"""

#### Gradient descent (LDA)

We can also use gradient descent to do LDA. The negative log likelihood is:


$$-\frac{1}{n}\mathcal L(\Phi) =- \frac{1}{n}\left \{\sum_{i=1}^n \sum_{k=1}^K {\mathbb{1}(y^{(i)}=k)} \cdot \ln \pi_k +\sum_{k=1}^K  \sum_{i=1}^n {\mathbb{1}(y^{(i)}=k)} \cdot \ln \mathcal{N}(x^{(i)}|\mu_k,\Sigma)\right \}$$

* almost the same as QDA but the covariance matrix is shared
"""
...
let
	...
	ϕ = randn(3)
	μs = randn(2, 3)
	## LDA model, the covariance is shared
	Ls = tril(zeros(2, 2))
	γ = 0.5
	losses = []
	for _ in 1:100
...
# ╔═╡ e5288b1b-4c0a-4aa3-b2c5-a36df988c348
md"""
...
plt2_pen_qda = let
	...
	if add_decision_boundary
		plot!(32:.1:60, 13:.1:22, (x, y) -> e_step([x, y]', pen_mvns, pen_πs)[1][:] |> argmax, lw=1.0,  st=:contourf, c=1:3, alpha=0.05, colorbar=false)
		idx = text_idx
